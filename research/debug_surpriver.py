...
def debug_surpriver():
    ...
    for symbol in symbols:
        ...
        ind = lib.conn.execute("SELECT * FROM calculated_indicators WHERE symbol = ? ORDER BY date DESC LIMIT 1", (symbol,)).df()
        if ind.empty:
        ...
        console.print(f"[*] Anomaly Score: {res.get('metrics', {}).get('Anomaly_Score', 0)}")
        console.print(f"[*] Active Windows: {res.get('metrics', {}).get('Active_Windows', '0/5')}")
