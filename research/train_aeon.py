...
def train_aeon():
    ...
    if not symbols:
        console.print("[error][!] No symbols found in NIFTY 50. Please sync constituents first.[/error]")
        return
    ...
    for sym in symbols:
        # Load historical prices with indicators
        df = lib.conn.execute(f"SELECT * FROM calculated_indicators WHERE symbol = '{sym}' ORDER BY date ASC").df()
        if len(df) > 100:
            all_dfs.append(df)
            
    if not all_dfs:
        console.print("[error][!] No historical data found in calculated_indicators. Run backfill first.[/error]")
        return
    ...
    if os.path.exists(model_path):
        try:
            ...
            if len(genes) == (480 * 16 + 16 + 16 * 4 + 4):
                ...
                console.print("[info][*] Loaded existing model for incremental training.[/info]")
    ...
    def reward_function(weights_list):
        ...
        fitness_scores = []
        # Sample a subset of the universe for each population evaluation to speed up
        sample_indices = np.random.choice(len(all_dfs), size=2, replace=False)
        
        for idx in sample_indices:
            env = SMCEnvironment(all_dfs[idx])
            fitness = env.evaluate_agent_vectorized(agent)
    ...
    initial_weights = [agent.weights['W1'], agent.weights['b1'], agent.weights['W2'], agent.weights['b2']]
    ...
    with Progress(
        ...
        for i in range(iterations):
            ...
            history.append({"iteration": i+1, "fitness": float(current_fitness)})
            ...
            progress.update(task, advance=1, description=f"[cyan]Evolving AEON Genes (Reward: {current_fitness:.2f})")
    ...
    with open(history_path, 'w') as f:
        import json
        json.dump(history, f)
    console.print(f"[success][✔] AEON Agent evolved and saved to {model_path}[/success]")
