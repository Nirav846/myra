...
def prepare_data(df, window_size=60):
    ...
    values = data.values
    # Optimized with list comprehension (Fix 73, 75: Avoid .append in loop)
    X = [values[i - window_size : i] for i in range(window_size, len(values))]
    y = [
        values[i, 3] for i in range(window_size, len(values))
    ]  # index 3 is 'close' pct_change
...
def train_cnn():
    console.print(
        "[bold magenta]--- [AEON DILATED CNN TRAINING (ML-2)] ---[/bold magenta]"
    )
    ...
    with Progress(
        ...
        # Optimized with list comprehension (Fix 109, 110: Avoid .append in loop)
        def _get_data(s):
            df = lib.conn.execute(
        ...
        results = [res for sym in symbols if (res := _get_data(sym))[0] is not None]
        all_X = [r[0] for r in results]
        all_y = [r[1] for r in results]

        # Manual progress update since we used comprehension
    ...
    # Train
    model.fit(
        X_train, y_train, epochs=10, batch_size=64, validation_split=0.1, verbose=1
    )
