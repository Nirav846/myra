...
def run_local_validation(symbol, target_breakout_date):
    ...
    # Performance Guard Compliant (Fix 13, 14)
    start_date = (
        (pd.to_datetime(target_breakout_date) - pd.DateOffset(days=100))
        .date()
        .isoformat()
    )
    end_date = (
        (pd.to_datetime(target_breakout_date) + pd.DateOffset(days=20))
        .date()
        .isoformat()
    )
    ...
    def dilated_trend(series, dilation, window=5):
        # Fix 36: Avoid .apply() on rolling windows
        vals = series.values
    ...
    sweet_spot = pre_move[
        pre_move["accumulation_score"] > pre_move["accumulation_score"].median() * 2.5
    ]
    if not sweet_spot.empty:
        print(
            sweet_spot[["date", "close", "accumulation_score", "confluence"]].to_string(
                index=False
            )
        )
    else:
    ...
    if not post_move.empty:
        # Fix 60: Avoid chained indexing
        base_df = df[df["date"] <= pd.to_datetime(target_breakout_date)]
        last_close = base_df["close"].iloc[-1]
        expansion = (post_move["close"].max() / last_close - 1) * 100
