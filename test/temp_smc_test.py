...
def calculate_structure(df, window=5):
    ...
    for i in range(len(df)):
        if (
            not np.isnan(df["swing_high"].iloc[i])
            and df["swing_high"].iloc[i] == df["high"].iloc[i]
        ):
            last_high = df["high"].iloc[i]
        if (
            not np.isnan(df["swing_low"].iloc[i])
            and df["swing_low"].iloc[i] == df["low"].iloc[i]
        ):
        ...
        if trend == 1:  # Uptrend
            ...
            if not np.isnan(last_high) and df["close"].iloc[i] > last_high:
                trend = 1
            if not np.isnan(last_low) and df["close"].iloc[i] < last_low:
                trend = -1
...
def test_smc():
    ...
    if fvg_count > 0:
        ...
        print(
            df[df["fvg"] != 0][["fvg", "fvg_top", "fvg_bottom"]].head()
        )  # noqa: performance
