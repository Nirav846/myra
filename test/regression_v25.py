...
def run_regression():
    console = Console()
    console.print("[bold cyan]--- [RADAR] MYRA v2.5 Reliability Regression ---[/bold cyan]")
    ...
    if len(df) > 756:
        console.print("[success][✔] Hybrid Data Loader successfully pulled 3+ years of history.[/success]")
    else:
        console.print(f"[error][✘] Hybrid Loader failed. Only got {len(df)} rows.[/error]")

    # Test 2: FVG Institutional Memory (3-Year Lookback)
    console.print("\n[*] Testing FVG Institutional Memory...")
    fvg = SMCManager.get_fvg_buy_zone(df)
    if fvg:
        console.print(f"[success][✔] FVG Identified: {fvg['bottom']:.2f} - {fvg['top']:.2f} (Mid: {fvg['mid']:.2f})[/success]")
    else:
        console.print("[warning][!] No unmitigated FVGs found in 3-year history for AXISBANK.[/warning]")
    ...
    try:
        ...
        latest = df.iloc[-1]
        console.print(f"[info][*] Latest Price: {latest['Close']:.2f} | SMA50: {latest['sma50']:.2f} | SMA200: {latest['sma200']:.2f}[/info]")
    ...
    if "metrics" in res:
        console.print(f"[success][✔] Surpriver v2 metrics generated: Anomaly={res['metrics']['Anomaly_Score']}[/success]")
    else:
