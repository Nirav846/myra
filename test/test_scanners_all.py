...
def test_all_scanners():
    ...
    for sid, name in scanners:
        ...
        if results:
            console.print(
                f"[bold green]PASS: Found {len(results)} matches.[/bold green]"
            )
            for r in results:
                console.print(
                    f"  - {r['Stock']}: LTP {r['LTP']}, Stars {r['Stars']}, Grade {r['Grade']}"
                )
        else:
            console.print(
                f"[yellow]OK: Scan finished, no current signals for {symbols}.[/yellow]"
            )
