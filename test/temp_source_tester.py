...
def test_endpoints():
    ...
    for t_date in test_dates:
        ds = t_date.strftime("%d%m%Y")
        ds_leg = t_date.strftime("%d%b%Y").upper()
        year = t_date.strftime("%Y")
        mon = t_date.strftime("%b").upper()
        
        print(f"\n--- Testing Date: {t_date.strftime('%Y-%m-%d')} ---")
        
        # 1. Test Primary (Archives Full)
        primary_url = f"https://archives.nseindia.com/products/content/sec_bhavdata_full_{ds}.csv"
        try:
        ...
        except Exception: print("Primary: ❌ FAILED")

        # 2. Test Fallback Price (Classic ZIP)
        zip_url = f"https://nsearchives.nseindia.com/content/historical/EQUITIES/{year}/{mon}/cm{ds_leg}bhav.csv.zip"
        try:
            r = requests.get(zip_url, headers=headers, timeout=5)
            status = "✅ ALIVE" if r.status_code == 200 else f"❌ DEAD ({r.status_code})"
            print(f"Fallback (Classic ZIP):     {status}")
        except Exception: print("Fallback ZIP: ❌ FAILED")
        ...
        except Exception: print("Fallback MTO: ❌ FAILED")
