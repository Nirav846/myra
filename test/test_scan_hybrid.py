...
def test_hybrid_scan():
    ...
    results = screener.execute_scan(
        "115", "Institutional Buying Test", portfolio_symbols=symbols
    )

    if results:
        console.print(
            f"[bold green]Success: Found {len(results)} candidates.[/bold green]"
        )
        for r in results:
            console.print(f"Stock: {r['Stock']}, LTP: {r['LTP']}, Stars: {r['Stars']}")
    else:
        console.print(
            "[yellow]Scan completed, but 0 candidates found (Logic working, but criteria not met).[/yellow]"
        )
