...
def debug_whale_internal():
    ...
    # Load prices
    df = conn.execute(
        f"SELECT * FROM prices WHERE symbol = '{symbol}' ORDER BY date ASC"
    ).df()
    ...
    m1 = XGBClassifier(
        n_estimators=30, max_depth=3, learning_rate=0.1, verbosity=0, random_state=42
    )
    ...
    latest_scaled = scaler.transform(latest_raw)  # noqa: performance
    ...
    print(
        f"Signal Conditions: Prob > 0.60 ({prob > 0.60}) AND Close > MA50 ({curr_c > ma50})"
    )
