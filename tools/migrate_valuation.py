...
def migrate_valuation():
    ...
    try:
        ...
        df = duck_conn.execute("SELECT symbol, pe, roe, NULL as eps, pb as book_value, market_cap, sector, CAST(last_updated AS VARCHAR) FROM fundamentals").df()
        if not df.empty:
            df = df.replace({np.nan: None})
            records = df.values.tolist()
            cursor.executemany("INSERT OR REPLACE INTO fundamentals VALUES (?,?,?,?,?,?,?,?)", records)
            sqlite_conn.commit()
    ...
    try:
        ...
        if 'quarterly_results' in tables:
            ...
            df = duck_conn.execute("SELECT symbol, CAST(report_date AS VARCHAR), revenue, net_profit, eps, opm_pct FROM quarterly_results").df()
            if not df.empty:
                df = df.replace({np.nan: None})
                records = df.values.tolist()
                cursor.executemany("INSERT OR REPLACE INTO quarterly_results VALUES (?,?,?,?,?,?)", records)
                sqlite_conn.commit()
