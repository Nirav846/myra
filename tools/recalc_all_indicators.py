...
# Add project root to path so we can import smc_calculator
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, PROJECT_ROOT)
from tools.smc_calculator import calculate_smc_indicators
...
def recalc_and_update():
    ...
    # 2. Ensure delivery column exists (for delivery MA)
    if 'delivery' not in df.columns:
        df['delivery'] = 0
    # Add a dummy delivery_qty column if needed (smc_calculator expects it)
    ...
    # 4. Write back to database in batches
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # Create a temporary table for updates
    df_calc[['symbol', 'date'] + update_cols].to_sql('temp_updates', conn, if_exists='replace', index=False)
    ...
    for col in update_cols:
        if col in df_calc.columns:
            sql = f"""
                ...
                SET {col} = (SELECT {col} FROM temp_updates 
