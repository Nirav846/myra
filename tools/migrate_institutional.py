...
def migrate_institutional():
    ...
    try:
        ...
        df_insider = duck_conn.execute(
            "SELECT symbol, person as acq_name, category, type, mode, value_cr, CAST(date AS VARCHAR) as date FROM insider_trades"
        ).df()
        if not df_insider.empty:
            df_insider = df_insider.replace({np.nan: None})
            records = df_insider.values.tolist()
            cursor.executemany(
                "INSERT OR IGNORE INTO insider_trades VALUES (?,?,?,?,?,?,?)", records
            )
            sqlite_conn.commit()
    ...
    try:
        ...
        df_deals = duck_conn.execute(
            "SELECT symbol, type, client, buy_sell, qty, price, CAST(date AS VARCHAR) as date FROM large_deals"
        ).df()
        if not df_deals.empty:
            df_deals = df_deals.replace({np.nan: None})
            records = df_deals.values.tolist()
            cursor.executemany(
                "INSERT OR IGNORE INTO large_deals VALUES (?,?,?,?,?,?,?)", records
            )
            sqlite_conn.commit()
