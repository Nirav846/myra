...
def run_mission_control_test():
    console = Console()
    console.print(
        "[bold cyan][MISSION CONTROL] Initiating End-to-End System Audit...[/bold cyan]"
    )
    ...
    console.print(
        "[*] Executing 'Super-Scan' (Growth + Momentum) on Active Universe..."
    )
    try:
        ...
        if results:
            console.print(
                f"[success][✔] Audit Successful! Identified {len(results)} high-conviction candidates.[/success]"
            )
            # Display top 5
            screener.rm.display_discovery_table(
                results[:5],
                "Mission Control: Top 5 Elite",
                "super_setup",
                ["RS_Raw", "Stage"],
            )
        else:
            console.print(
                "[warning][!] Audit Complete: 0 candidates found, but data flow is VERIFIED.[/warning]"
            )
