...
def debug_dcal_insertion():
    ...
    if found.empty:
        ...
        for _, row in found.iterrows():
            ...
            res = conn.execute("SELECT * FROM technical_data WHERE symbol = ? AND date = ?", (current_sym, test_date)).fetchone()
            conn.close()
            if res:
                print(f"    - [!] Row already exists in technical.db for {current_sym} on {test_date}")
            else:
                print(f"    - [ ] Row MISSING in technical.db for {current_sym} on {test_date}")
