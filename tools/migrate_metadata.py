...
def migrate_metadata():
    ...
    try:
        ...
        df = duck_conn.execute(
            "SELECT symbol, CAST(first_seen AS VARCHAR), CAST(last_seen AS VARCHAR), in_active_universe, in_nifty500, NULL as sector, is_active FROM symbols_master"
        ).df()
        if not df.empty:
            ...
            cursor.executemany(
                "INSERT OR REPLACE INTO symbols_master VALUES (?,?,?,?,?,?,?)", records
            )
            sqlite_conn.commit()
    ...
    try:
        ...
        if not df.empty:
            ...
            records = df.values.tolist()
            cursor.executemany(
                "INSERT OR REPLACE INTO index_constituents VALUES (?,?)", records
            )
            sqlite_conn.commit()
    ...
    try:
        ...
        df = duck_conn.execute(
            "SELECT symbol, CAST(date AS VARCHAR) as date, close FROM benchmark"
        ).df()
        if not df.empty:
            df = df.replace({np.nan: None})
            records = df.values.tolist()
