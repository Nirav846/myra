...
def trace_multibagger_v6():
    ...
    for sym in test_symbols:
        ...
        console.print(
            f"  - 60D Base Range: {round(base_range_pct*100, 2)}% (Target: < 25%)"
        )
        ...
        console.print(
            f"  - Short ATR: {round(atr_short, 2)} vs Long ATR: {round(atr_long, 2)} (Compressing: {atr_short < atr_long})"
        )
        ...
        if res["signal"]:
            # Fix 58: Avoid chained indexing
            metrics = res["metrics"]
            score = metrics["Score"]
            console.print(f"  [green]SUCCESS: Signal Generated! Score: {score}[/green]")
        else:
            console.print(
                f"  [red]FAIL: {res['reason']} (Score: {res.get('score', 'N/A')})[/red]"
            )
