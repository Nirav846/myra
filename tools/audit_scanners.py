...
def run_suite_audit():
    console = Console()
    console.print("[bold cyan][AUDIT] Initiating Multi-Scanner Data Mapping Audit...[/bold cyan]")
    
    lib = Librarian(read_only=True)
    screener = MYRAScreener(console)
    
    scanners = [
        {"id": "whale_tracker", "name": "Whale Tracker (Large Deals)", "cols": ["Stars", "Inst_Intensity", "AV_Total"]},
        {"id": "bottom_hunter", "name": "Bottom Hunter (FVG Reversal)", "cols": ["Stars", "FVG_Zone", "Support"]},
        {"id": "surpriver_v2", "name": "NSE Surpriver v2 (Delivery Z)", "cols": ["Stars", "RDV", "Money_Flow"]}
    ]
