...
*(You can modify this live in the Dashboard's **Settings Config** tab).*
...
```
*Valid `database` targets match your sidecar names:* `_tech_conn`, `_inst_conn`, `_meta_conn`, `_gov_conn`.
...
- `ticker` (TEXT)
- `sector` (TEXT) - *Used for Grouping Sector Flow*
- `current_price` (REAL)
- `graham_value` (REAL)
