...
const matchComputed = content.match(regexComputed);
const matchDestructure = content.match(regexDestructure);

if (matchComputed && matchDestructure) {
    content = content.replace(matchComputed[0], allMath);
    content = content.replace(matchDestructure[0], newDestructure);
    fs.writeFileSync(path, content, 'utf8');
    console.log("SUCCESS!");
} else {
    console.error("FAILED to find patterns.");
}
