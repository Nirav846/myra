...
def run_2026_ingest():
    console = Console()
    console.print(
        "[bold cyan]--- [FUNDAMENTAL INGESTOR] Starting 2026 Optimization ---[/bold cyan]"
    )
    ...
    if "--reset" in sys.argv:
        console.print(
            "[warning][!] Resetting sync markers for all active stocks...[/warning]"
        )
        conn = lib._meta_conn
        if conn:
            conn.execute(
                "UPDATE symbols_master SET last_fundamental_update = '1900-01-01'"
            )
            conn.commit()
        else:
    ...
    if not stale_stocks:
        console.print(
            "[success][✔] All stocks are current. No update needed.[/success]"
        )
        return
