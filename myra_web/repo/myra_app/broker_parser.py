    ...
    def parse_holdings(self) -> list:
        ...
        try:
            ...
            for col in df.columns:
                c_low = str(col).lower()
                if (
                    "symbol" in c_low or "instrument" in c_low
                ) and "Stock" not in final_map:
                    final_map[col] = "Stock"
                elif ("qty" in c_low or "quantity" in c_low) and "Qty" not in final_map:
                    final_map[col] = "Qty"
                elif (
                    "avg" in c_low or "buy" in c_low or "cost" in c_low
                ) and "Avg_Price" not in final_map:
