...
def precompute_ranks(df):
    ...
    if "smart_money_score" in df.columns:
        df["_rank_sm_score"] = df["smart_money_score"].rank(
...
def delivery_score(row):
    ...
    if pd.isna(d_rank) or d_rank is None:
        d_rank = 0.3
    if pd.isna(sm_rank) or sm_rank is None:
        sm_rank = 0.3
...
def fundamental_score(row):
    ...
    if pd.isna(roe_score):
        roe_score = 0.4
    if pd.isna(growth_score):
        growth_score = 0.4
...
def valuation_score(row):
    ...
    if mos > 0:
        return 0.8  # Fairly Valued
    if mos > -0.2:
        return 0.5  # Slightly Overvalued
