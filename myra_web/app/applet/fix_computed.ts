...
const computed = useMemo(() => {
    ...
    return {
        dates, opens, highs, lows, closes, volumes, vwap, deliveryFinal, deliveryPct, deliveryRatio, stockReturn, volComp, relVol, divScores, niftyOut, trendAlignment, volumeColors, deliveryColorsInverse,
        smasTraces, rsiTraces, volProfileTraces, vwapTraces, swingsTraces, instBlocksTraces, delVwapBandsTraces, delAdTraces, niftyOutTraces, smartMoneyPrintsTraces, delIntensityCoreTraces, vpMaxVolume, delMaData, shapes, volumeTraces, deliveryTraces, annotations,
        currentY, rsiDomain, delAdDomain, delDomain, volDomain, priceDomain
    };
}, [indicators, viewport]);`

const matchOldComputed = content.match(/const computed = useMemo\(\(\) => \{[\s\S]*?\}, \[data, toggles, viewport\]\);/);
if (matchOldComputed) {
    content = content.replace(matchOldComputed[0], allMath);
    fs.writeFileSync(path, content, 'utf8');
    console.log("Success");
} else {
    console.error("Could not find computed block");
}
