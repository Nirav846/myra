...
def force_backfill():
    console = Console()
    console.print(
        "[bold cyan][MYRA] Initializing Force Backfill (1.5 Years)...[/bold cyan]"
    )
    ...
    try:
        ...
        console.print(
            "[info][*] Fetching 1.5 years of history... This may take several minutes.[/info]"
        )
        ...
        res = lib.conn.execute(
            "SELECT symbol, COUNT(*) as cnt FROM prices WHERE symbol IN ('SUNPHARMA', 'RELIANCE') GROUP BY symbol"
        ).fetchall()
        for r in res:
