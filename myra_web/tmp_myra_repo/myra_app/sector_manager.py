...
class SectorManager:
        ...
        for source in urls:
            try:
                ...
                if r and r.status_code == 200:
                    ...
                    sec_col = next(
                        (c for c in df.columns if c.strip() == "Sector"),
                    ...
                    ind_col = next(
                        (
                            c
                            for c in df.columns
                            if "INDUSTRY" in c.upper() and "BASIC" not in c.upper()
                        ),
                        None,
                    )
    ...
    def update_symbol(self, symbol, data):
        ...
        try:
            # Check if locked
            res = conn.execute(
                "SELECT sector_locked FROM symbols_master WHERE symbol = ?", (symbol,)
            ).fetchone()
            if res and res[0] == 1:
                return False
    ...
    def fetch_nse_meta_bulk(self, symbols):
        ...
        def fetch_single(symbol):
            try:
                ...
                if r.status_code == 200:
                    ...
                    if ind_info:
                        return symbol, {
                            "raw_sector": ind_info.get(
                                "macro", ind_info.get("sector", "Unknown")
                            ),
        ...
        print(
            f"[MYRA] Launching Parallel NSE Meta Fetcher for {len(symbols)} symbols..."
        )
    ...
    def incremental_sync(self):
        ...
        print(
            f"[MYRA] Incremental Sync: Found {len(stale_symbols)} symbols to refresh."
        )
    ...
    def _batch_update_symbols(self, symbols, data_map):
        ...
        updated = 0
        try:
            # Optimized with list comprehension (Fix 380: Avoid .append in loop)
            def _to_record(s):
                data = data_map.get(s)
                if not data:
                    return None
                return (
                    self.normalize(data["raw_sector"]),
                    self.normalize(data["raw_industry"]),
                    data["raw_sector"],
                    data["raw_industry"],
                    data["source"],
                    data["confidence"],
            ...
            records = [r for s in symbols if (r := _to_record(s)) is not None]
