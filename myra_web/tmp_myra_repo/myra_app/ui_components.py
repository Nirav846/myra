...
def get_logo_panel():
    return Panel(MYRA_LOGO, border_style="magenta", box=DOUBLE, padding=(1, 2))
...
def get_categorized_menu(strategies: dict):
    ...
    for cat_name, ids in cats.items():
        ...
        categorized[cat_name] = [
            (s_id, _get_name(s_id)) for s_id in ids if s_id in strategies
        ]
...
def render_categorized_menu(console, categorized_menu):
    ...
    console.print(
        Panel(
            table,
            title="[bold white]Main Menu Options[/bold white]",
            border_style="blue",
        )
    )
