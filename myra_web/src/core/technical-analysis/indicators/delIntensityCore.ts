...
export const delIntensityCoreIndicator: IndicatorModule<
  any,
  DelIntensityCoreResult
> = {
  id: "delIntensityCore",
  ...
  calculate: (data, config, context) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let sum = 0;
      for (let j = 0; j < 60; j++)
        sum += data[i - j].delivery_final
          ? Number(data[i - j].delivery_final)
          : Number(data[i - j].delivery) || 0;
      avgDel60.push(sum / 60);
    }

    for (let i = 0; i < data.length; i++) {
      const d = data[i];
      const delQty = d.delivery_final
