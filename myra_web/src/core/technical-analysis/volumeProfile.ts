...
export const calculateVolumeProfile = (
  ...
  if (config.resolution === "weekly" || config.resolution === "monthly") {
    ...
    visibleData.forEach((d: any) => {
      ...
      if (!aggregatedMap.has(key)) {
        ...
        agg.delivery_final =
          (agg.delivery_final || 0) + (d.delivery_final || d.delivery || 0);
        agg.volume_final =
          (agg.volume_final || 0) + (d.volume_final || d.volume || 0);
  ...
  const vMinL = Math.min(
    ...visibleData.map((d) => (typeof d.low === "number" ? d.low : Infinity)),
  );
  const vMaxH = Math.max(
    ...visibleData.map((d) =>
      typeof d.high === "number" ? d.high : -Infinity,
    ),
  );
  ...
  let pocVolY =
    pocVolBin !== -1 ? vMinL + pocVolBin * binSize + binSize / 2 : null;
  let pocDelY =
    pocDelBin !== -1 ? vMinL + pocDelBin * binSize + binSize / 2 : null;
