...
import { VolumeProfileResult, VolumeProfileConfig } from '../../technical-analysis/volumeProfile';

export const volumeProfileTraceBuilder: TraceBuilder<VolumeProfileResult, VolumeProfileConfig> = {
    id: 'volumeProfile',
    ...
    buildShapes: (result, context, config?: VolumeProfileConfig) => {
        ...
        if (config?.showDeliveryProfile !== false && result.pocVolY !== null) {
            shapes.push({
                type: 'line', xref: 'paper', x0: 0, x1: 1, yref: 'y', y0: result.pocVolY, y1: result.pocVolY,
        ...
        if (config?.showDeliveryProfile !== false && result.pocDelY !== null) {
            shapes.push({
                type: 'line', xref: 'paper', x0: 0, x1: 1, yref: 'y', y0: result.pocDelY, y1: result.pocDelY,
        ...
        if (config?.showDeliverySR !== false) {
            ...
            for (let i = 2; i < bins - 2; i++) {
                if (result.deliveryProfileX[i] > result.deliveryProfileX[i-1] && 
