...
export default function App() {
  ...
  const activeTab =
    TABS.find((t) => location.pathname.startsWith(t.path))?.id ||
    (location.pathname === "/settings" ? "Settings" : "Mission Control");

  const [globalSelectedTicker, setGlobalSelectedTicker] = useState<
    string | undefined
  >();
  ...
  const disconnectedDBs = Object.entries(
    health as Record<string, HealthStatus>,
  ).filter(([_, status]) => !status.connected);

  // Dynamic Theme mappings based on SettingsContext
  const bgMain =
    settings.theme === "pitch-black" ? "bg-[#000000]" : "bg-[#0e1117]";
  const bgSidebar =
    settings.theme === "pitch-black" ? "bg-[#0a0a0a]" : "bg-[#262730]";
  const bgFooter =
    settings.theme === "pitch-black" ? "bg-[#050505]" : "bg-[#0e1117]";
  const densityClass =
  ...
  return (
    <div
      className={`flex h-screen w-full ${bgMain} text-[#fafafa] font-sans overflow-hidden transition-colors relative`}
    >
      <AlertManager />
      <DebugPanel />
      {isMobileMenuOpen && (
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`${isDesktopMenuExpanded ? "w-64" : "w-16"} ${bgSidebar} flex flex-col border-[#ffffff1a] absolute md:relative z-40 h-full !flex transform transition-all duration-300 ${settings.sidebarPosition === "Right" ? "border-l order-last right-0" : "border-r left-0"} ${isMobileMenuOpen ? "translate-x-0" : settings.sidebarPosition === "Right" ? "translate-x-full md:translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        <div
          className={`p-2 border-b border-[#ffffff1a] flex flex-col items-center`}
        >
          <div
            className={`flex items-center justify-between w-full h-6 ${!isDesktopMenuExpanded && "justify-center"}`}
          >
            {isDesktopMenuExpanded && (
              <div className="flex items-center gap-2 overflow-hidden">
                <div
                  className={`w-6 h-6 rounded-md flex items-center justify-center font-bold italic text-white flex-shrink-0 transition-colors ${settings.animations ? "hover:scale-105" : ""} ${accent.bg600}`}
                >
                  <Command size={14} />
                </div>
                <h1 className="text-sm font-bold tracking-tight whitespace-nowrap">
                  MYRA <span className={`${accent.text400}`}>v3.2</span>
                </h1>
              </div>
            )}
            {!isDesktopMenuExpanded && (
            ...
            <div className="flex items-center gap-1">
              {isDesktopMenuExpanded && (
                <button
                  className="hidden md:flex p-1 text-[#888] hover:text-white transition-colors"
                  onClick={() => setIsDesktopMenuExpanded(false)}
          ...
          {isDesktopMenuExpanded && (
            <div
              className={`flex items-center gap-1.5 mt-2 px-2 py-1 ${bgMain} rounded border border-[#ffffff1a] w-full`}
            >
              <div
                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${disconnectedDBs.length > 0 ? "bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.6)]" : "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"}`}
              ></div>
              <span className="text-[9px] uppercase font-mono tracking-widest text-[#888] whitespace-nowrap overflow-hidden text-ellipsis">
                Librarian {disconnectedDBs.length > 0 ? "Degraded" : "Active"}
              </span>
            </div>
          )}
          {!isDesktopMenuExpanded && (
            <div className={`mt-2 flex justify-center items-center h-2 w-2`}>
              <div
                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${disconnectedDBs.length > 0 ? "bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.6)]" : "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"}`}
                title={`Librarian ${disconnectedDBs.length > 0 ? "Degraded" : "Active"}`}
              ></div>
            </div>
          )}
        </div>
        ...
        <nav
          className="flex-1 py-4 flex flex-col space-y-1 overflow-x-hidden overflow-y-auto w-full px-2"
          style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
        >
          {isDesktopMenuExpanded ? (
            <div className="px-3 py-2 text-[10px] font-semibold text-[#888] uppercase tracking-wider whitespace-nowrap">
              Navigation
            </div>
          ) : (
          ...
          {isDesktopMenuExpanded && (
            <div className="space-y-2 mt-2 px-3">
              {Object.entries(
                librarian.health as Record<string, HealthStatus>,
              ).map(([dbName, status]) => (
                <div
                  key={dbName}
                  className="flex justify-between items-center text-[10px] font-mono"
                >
                  <span className="text-[#666] whitespace-nowrap overflow-hidden text-ellipsis mr-2">
                    {dbName.toUpperCase()}
                  </span>
                  <span className="flex items-center gap-1.5 shrink-0">
                    ...
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${status.connected ? "bg-green-400" : "bg-red-400"} ${settings.animations ? "animate-pulse" : ""}`}
                    />
                  </span>
                </div>
              ))}
        ...
        <div
          className={`mt-auto flex flex-col border-t border-[#ffffff1a] py-2 shrink-0`}
        >
          ...
          >
            ...
            {isDesktopMenuExpanded && (
              <span className="whitespace-nowrap">Settings Config</span>
            )}
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <div
          className={`flex-1 overflow-x-hidden overflow-y-auto flex flex-col ${densityClass}`}
        >
            ...
            <div className="flex items-center gap-2 h-full">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <p className="text-[10px] text-[#888] hidden sm:block">
                  Librarian v3.2: Myra React Bridge
                </p>
              </div>
            </div>
            <div className="px-4">
          ...
          {disconnectedDBs.map(([dbName, status]) => (
            <div
              key={dbName}
              className="bg-red-950/40 border border-red-500/50 p-4 rounded-lg flex items-start gap-3 shrink-0"
            >
              <AlertCircle
              ...
              <div>
                <h3 className="text-red-400 text-sm font-semibold mb-1">
                  Error: Database Missing ({dbName})
                </h3>
                <p className="text-[#ccc] text-xs font-mono">
          ...
          <div className="flex-1">
            <div className="max-w-6xl">
              <Routes>
                ...
                <Route
                  path="/leaderboard"
                  element={<LeaderboardView lib={librarian} />}
                />
                <Route
                  path="/fvg-scanner"
                  element={<FVGScannerView lib={librarian} />}
                />
                <Route
                  path="/historical-search"
                  element={<HistoricalSearchView lib={librarian} />}
                />
                <Route
                  path="/chart"
                  element={
                    <AdvancedChartView
                      lib={librarian}
                      activeSymbol={globalSelectedTicker}
                    />
                  }
                />
                <Route
                  path="/sector-flow"
                  element={<SectorFlowView lib={librarian} />}
                />
                <Route
                  path="/reversion-engine"
                  element={
                    <ReversionEngineView
                      lib={librarian}
                ...
                />
                <Route
                  path="/ghost-simulator"
                  element={<GhostSimulatorView lib={librarian} />}
                />
                <Route
                  path="/multibagger-matrix"
                  element={<MultibaggerMatrixView lib={librarian} />}
                />
                <Route
                  path="/inst-dom"
                  element={<InstDOMView lib={librarian} />}
                />
                <Route
                  path="/parquet-lake"
                  element={<DataLakeView lib={librarian} />}
                />
          ...
          {isDesktopMenuExpanded && (
            <div
              className={`shrink-0 h-24 ${bgSidebar} border-t border-[#ffffff1a] p-2 flex flex-col font-mono text-[10px] text-[#aaa] overflow-y-auto`}
            >
              {logs.map((log, idx) => (
        ...
        <footer
          className={`h-10 ${bgFooter} border-t border-blue-500/30 shrink-0 flex items-center justify-between px-4 font-mono text-[11px] shadow-[0_-2px_10px_rgba(59,130,246,0.05)] transition-colors`}
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-cyan-400">
                Fund:{" "}
                {pipelineStatus
                  ? formatDate(pipelineStatus.fundamentals)
                  : "N/A"}
              </span>
              <span className="text-white font-bold ml-1">ETF: </span>
              <span className="text-cyan-400">
                {pipelineStatus ? formatDate(pipelineStatus.etf) : "N/A"}
              </span>
              <span className="text-white font-bold ml-1">IDX: </span>
              <span className="text-cyan-400">
                {pipelineStatus ? formatDate(pipelineStatus.index) : "N/A"}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div
              className={`w-1.5 h-1.5 rounded-full ${pipelineStatus && isToday(pipelineStatus.ingest) ? "bg-green-400" : "bg-yellow-400"} ml-0.5 ${settings.animations ? "animate-pulse" : ""}`}
            ></div>
          </div>
        </footer>
      </main>
