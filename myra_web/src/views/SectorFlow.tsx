    ...
    try {
      ...
      if (symbolsResult) {
        for (const m of symbolsResult) {
          ...
          const normalizedSector =
            m.sector && m.sector.trim() !== "" ? m.sector : "Uncharted Sector";
      ...
      if (techResult && techResult.length > 0) {
        ...
        for (const t of techResult) {
          const meta = metaMap.get(t.ticker) || {
            sector: "Uncharted Sector",
            bucket: "Deep Frontier",
          };
        ...
        const finalData = Object.keys(flowMap)
          .map((key) => ({
            name: key,
            flow: flowMap[key].capital_flow,
            avg_del:
              flowMap[key].count > 0
                ? flowMap[key].sum_delivery / flowMap[key].count
                : 0,
          }))
  ...
  const generateMockData = () => {
    ...
    if (groupBy === "sector") {
      ...
      setData([
        ...
        {
          name: "Broader Market (N500)",
          flow: 2100000000 * scale,
          avg_del: 60.1,
        },
      ...
      <div className="p-6 flex flex-col gap-6">
        ...
        <div className="bg-[#0e1117] border border-[#ffffff0a] p-4 rounded-lg flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-sm">
              Net Capital Flow (Custom Range)
            </h4>
            <p className="text-xs text-[#888] mt-1 pr-6">
              Measures total capital absorbed by institutional delivery across
              correlated sectors or indices. Darker bars indicate higher
              underlying delivery percentages over the selected period.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4 justify-end text-xs font-mono text-[#666]">
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 bg-cyan-400 rounded-sm"></div> Strong
            Accumulation (&gt;60% Del)
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 bg-cyan-700 rounded-sm"></div> Weak
            Absorption
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 bg-red-500 rounded-sm"></div> Capital
            Outflow
          </div>
        </div>
      </div>
