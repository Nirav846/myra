      ...
      if (result && result.length > 0) {
        ...
        const mappedData = result
          .map((r: any) => {
            ...
            const priceApprox =
              r.pe && r.eps ? Number(r.pe) * Number(r.eps) : 0;

            const mos =
              graham > 0 ? ((graham - priceApprox) / graham) * 100 : 0;
            return {
    ...
    const dataString = apiData
      ? "Ticker\tSector\tPrice\tGraham Value\tMoS (%)\n" +
        apiData
          .map(
            (r) =>
              `${r.ticker}\t${r.sector}\t${r.price}\t${r.graham}\t${r.mos}%`,
          )
          .join("\n")
      : "";
        ...
        <div className="flex justify-between items-center gap-4 bg-[#0e1117] border border-[#ffffff0a] p-3 rounded flex-wrap">
          <div className="flex gap-4 items-center">
            <div className="flex flex-col">
              <label className="text-[10px] text-[#888] font-mono uppercase mb-1">
                Min MoS (%)
              </label>
              <input
                type="number"
                ...
                className="w-20 bg-[#1a1c24] border border-[#ffffff1a] rounded px-2 py-1 text-xs text-[#fafafa] font-mono focus:border-cyan-500 outline-none"
              />
            </div>
            <div className="flex flex-col">
              <label className="text-[10px] text-[#888] font-mono uppercase mb-1">
              ...
              <select
                ...
                className="w-20 bg-[#1a1c24] border border-[#ffffff1a] rounded px-2 py-1 text-xs text-[#fafafa] font-mono focus:border-cyan-500 outline-none"
              >
                <option className="bg-[#1a1c24] text-[#fafafa]" value={5}>
                  5
                </option>
                <option className="bg-[#1a1c24] text-[#fafafa]" value={10}>
                  10
                </option>
                <option className="bg-[#1a1c24] text-[#fafafa]" value={15}>
                  15
                </option>
                <option className="bg-[#1a1c24] text-[#fafafa]" value={50}>
                  50
                </option>
              </select>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {isDemo && (
              <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-1 rounded font-mono">
                ⚠️ DEMO DATA
              </span>
            )}
            <button
            ...
            <tbody className="text-[#ccc]">
              {dataLoaded &&
                ...
                apiData.map((row, idx) => (
                  ...
                  >
                    <td className="py-2 px-2 text-[#fafafa] font-bold">
                      {row.ticker}
                    </td>
                    ...
                    <td className="py-2 px-2 text-right text-blue-400">
                      {row.graham.toFixed(2)}
                    </td>
                    <td className="py-2 px-2 text-right text-green-400">
                      +{row.mos}%
                    </td>
                    <td className="py-2 px-2 text-right">
                      <span className="bg-green-500/10 text-green-400 px-1.5 py-0.5 rounded text-[10px]">
                        {row.rating}
                      </span>
                    </td>
                  </tr>
