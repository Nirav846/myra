...
import {
  computeExhaustionSignal,
  computeDivergenceSignal,
  computeSpringCoilSignal,
  SignalData,
} from "../lib/reversionSignals";

type SetupType = "Exhaustion" | "Divergence" | "SpringCoil";

export default function ReversionEngineView({
  lib,
  onNavigate,
}: {
  lib: Librarian;
  ...
  const [metadataMap, setMetadataMap] = useState<
    Map<string, { sector: string; indices: string[]; in_nifty500: number }>
  >(new Map());
  ...
  const fetchMetadata = async () => {
    try {
      ...
      const symbolsResult = await lib.executeQuery(
        "_meta_conn",
        "SELECT symbol, sector, in_nifty500 FROM symbols_master LIMIT 5000",
      );
      ...
      const map = new Map<
        string,
        { sector: string; indices: string[]; in_nifty500: number }
      >();
  ...
  const fetchData = async (setup: SetupType) => {
    ...
    try {
      ...
      const result = await lib.executeQuery(
        "_tech_conn",
        `
        WITH RecentDates AS (
  ...
  const processSetups = (rawData: any[], setup: SetupType) => {
    ...
    const parsedData: SignalData[] = rawData.map((r) => ({
      ...
      avg_close_sq_20:
        Number(r.avg_close_sq_20) ||
        Number(r.Close ?? r.close) * Number(r.Close ?? r.close),
    ...
    const finalData = processed
      .map((item) => {
        const meta = metadataMap.get(item.ticker) || {
          sector: "Uncharted Sector",
          indices: [],
          in_nifty500: 0,
        };
        ...
        if (
          ...
          meta.indices.some((i: string) => i.includes("NIFTY NEXT 50"))
        ) {
          bucket = "Large Cap (N100)";
        } else if (
          meta.in_nifty500 === 1 ||
          meta.indices.some((i: string) => i.includes("NIFTY 500"))
        ) {
  ...
  const generateMockData = (setup: SetupType) => {
    ...
    const mapped = symbols
      .map((sym) => {
        ...
        if (sym === "BAJFINANCE" || sym === "KOTAKBANK")
          bucket = "Large Cap (N100)";
  ...
  return (
    <div className="bg-[#0b0c10] rounded-xl border border-white/10 shadow-2xl flex flex-col overflow-hidden max-w-5xl mx-auto">
      <div className="p-4 border-b border-white/5 bg-gradient-to-r from-cyan-900/20 to-transparent flex justify-between items-center">
        ...
        <span className="text-[10px] font-mono text-cyan-500/70 border border-cyan-500/20 px-2 py-0.5 rounded-full bg-cyan-500/5">
          v1.2 Quantum
        </span>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-[#12141a] border border-white/5 p-3 rounded-lg shadow-sm">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto h-full">
            <div className="flex bg-[#0A0A0A] rounded border border-[#333333] shadow-inner items-center h-[34px] overflow-hidden">
              ...
              >
                <option value="All" className="bg-[#1a1c24] text-white">
                  All
                </option>
                <option
                  value="Large Cap (N50)"
                  className="bg-[#1a1c24] text-white"
                >
                  Large Cap (N50)
                </option>
                <option
              ...
              >
                ...
                {availableSectors.map((s) => (
                  <option key={s} value={s} className="bg-[#1a1c24] text-white">
                    {s}
                  </option>
                ))}
                {isDemo && availableSectors.length === 0 && (
                  <>
                    <option value="IT" className="bg-[#1a1c24] text-white">
                      IT
                    </option>
                    <option
                      value="Financials"
                    ...
                    <option value="Energy" className="bg-[#1a1c24] text-white">
                      Energy
                    </option>
                  </>
                )}
            ...
            {errorMsg && (
              <span className="text-xs text-red-500 font-mono px-2 py-1 bg-red-500/10 border border-red-500/20 rounded hidden lg:block">
                {errorMsg}
              </span>
            )}
            {isDemo && (
              <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-1 rounded border border-yellow-500/20 font-mono hidden lg:block">
                ⚠️ DEMO
              </span>
            )}
            <button
        ...
        <div className="bg-[#12141a]/50 border border-white/5 rounded-lg p-4 text-xs font-mono text-[#888] leading-relaxed shadow-sm">
          {activeSetup === "Exhaustion" && (
            <p>
              ...
              Looks for isolated instances where price drops intensely, volume
              explodes, but delivery percentage implies institutional
              accumulation rather than pure panic offloading.
            </p>
          )}
          {activeSetup === "Divergence" && (
            <p>
              ...
              Scans for symbols testing multi-month lows with average volume,
              but experiencing extreme anomalies in Delivery % (e.g. {">"} 85%),
              indicating quiet smart-money absorption.
            </p>
          )}
          {activeSetup === "SpringCoil" && (
            <p>
              ...
              Identifies assets going dormant. Both price range and volume dry
              up dramatically, while delivery percentage remains stable,
              preceding explosive expansion.
            </p>
          )}
        </div>

        <div
          className={`overflow-x-auto relative group transition-opacity duration-300 ${isRefreshing ? "opacity-50" : "opacity-100"} min-h-[250px] bg-[#12141a] rounded-lg border border-white/5`}
        >
          <table className="w-full text-left font-mono text-xs">
            <thead>
              <tr className="text-[#666] border-b border-white/5 bg-black/20">
                <th className="py-3 px-3 font-semibold uppercase min-w-[100px] tracking-wider rounded-tl-lg">
                  Ticker
                </th>
                <th className="py-3 px-3 font-semibold uppercase tracking-wider">
                  Sector
                </th>
                <th className="py-3 px-3 font-semibold uppercase text-right tracking-wider">
                  Close (₹)
                </th>
                <th className="py-3 px-3 font-semibold uppercase text-right tracking-wider">
                  Score
                ...
                <th className="py-3 px-3 font-semibold uppercase w-[25%] hidden md:table-cell tracking-wider">
                  Action Plan
                </th>
                <th className="py-3 px-3 font-semibold uppercase w-[20%] hidden lg:table-cell tracking-wider rounded-tr-lg">
                  Engine Note
                </th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="py-3 px-3">
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() =>
                            onNavigate?.("Technical Chart", row.ticker)
                          }
                        ...
                        {row.isUncharted && (
                          <span className="text-[8px] bg-red-500/10 border border-red-500/20 text-red-400 font-bold px-1.5 py-0.5 rounded tracking-wide">
                            UNCHARTED
                          </span>
                        )}
                      </div>
                      <div className="text-[9px] text-[#666] tracking-widest uppercase truncate mt-0.5">
                        {row.bucket}
                      </div>
                    </td>
                    <td className="py-3 px-3 text-[#888] truncate max-w-[120px]">
                      {row.sector}
                    </td>
                    <td className="py-3 px-3 text-right text-[#fafafa] font-medium">
                      {row.close.toFixed(2)}
                    </td>
                              ...
                              backgroundColor:
                                row.score > 90
                                  ...
                                  : row.score > 80
                                    ? "#3b82f6"
                                    : "#8b5cf6",
                              boxShadow: `0 0 8px ${row.score > 90 ? "#22c55e" : row.score > 80 ? "#3b82f6" : "#8b5cf6"}`,
                            }}
                          ></div>
                        </div>
                        <span
                          className={`font-bold ${row.score > 90 ? "text-green-400" : row.score > 80 ? "text-blue-400" : "text-purple-400"}`}
                        >
                          {row.score.toFixed(1)}
                        </span>
                      </div>
                    ...
                    <td className="py-3 px-3 text-right text-gray-400 font-medium">
                      {row.delPerc.toFixed(1)}%
                    </td>
                    <td className="py-3 px-3 hidden md:table-cell">
                      <div className="flex flex-col gap-1 text-[10px] font-mono">
                        <div className="text-[9px] text-[#888] uppercase tracking-wider mb-0.5">
                          T+1 Buy Stop
                        </div>
                        <div className="flex items-center flex-wrap gap-x-3 gap-y-1 bg-black/40 p-1.5 rounded-md border border-white/5 shadow-inner">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-gray-300">
                              En{" "}
                              <span className="text-green-400 font-bold text-xs ml-0.5">
                                ₹{row.entry.toFixed(2)}
                              </span>
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-gray-300">
                              SL{" "}
                              <span className="text-red-400 font-bold text-xs ml-0.5">
                                ₹{row.sl.toFixed(2)}
                              </span>
                            </span>
                          </div>
                          ...
                          <span className="text-orange-400 font-bold">
                            R {row.risk.toFixed(1)}%
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-3 text-[#777] italic hidden lg:table-cell truncate max-w-[150px]">
                      {row.note}
                    </td>
                  </tr>
                ))}
