...
export default function FVGScannerView({ lib }: { lib: Librarian }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      await lib.executeQuery('_tech_conn', 'SELECT 1');
       ...
       // if it fails, it fails. We just continue for now
    }
    ...
    setTimeout(() => {
      ...
      alertBus.emit({
          ...
          // DAILY-ADJUSTED: Changed 4H to 1D to reflect daily data capabilities
          message: "Bullish divergence FVG logged for BTC on 1D",
          level: "info",
          source: "FVG Scanner"
      });
    }, 600);
  ...
  const handleCopy = () => {
    // DAILY-ADJUSTED: Changed timeframes from 15m/1H/4H to 1D/1W/1M to reflect daily data capabilities
            ...
            <tbody className="text-[#ccc]">
              {dataLoaded && (
                <>
                  ...
                  >
                    {/* DAILY-ADJUSTED: Changed from 15m to 1D */}
                    <td className="py-2 px-2">1D</td>
                  ...
                  >
                    {/* DAILY-ADJUSTED: Changed from 1H to 1W */}
                    <td className="py-2 px-2">1W</td>
                  ...
                  >
                    {/* DAILY-ADJUSTED: Changed from 4H to 1M */}
                    <td className="py-2 px-2">1M</td>
