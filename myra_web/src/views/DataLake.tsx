        ...
        >
          ...
          <table className="w-full text-left font-mono text-xs">
            <thead>
              <tr className="text-[#888] border-b border-[#ffffff1a]">
                <th className="pb-2 px-2 font-medium uppercase min-w-[80px]">
                  Date
                </th>
                  ...
                  {category === "Smart Money Concepts" ? (
                    <>
                      <td className="py-2 px-2 text-green-400">
                        {row.bullish_fvg ? "Detected" : "-"}
                      </td>
                      <td className="py-2 px-2 text-red-400">
                        {row.bearish_fvg ? "Detected" : "-"}
                      </td>
                      <td className="py-2 px-2 text-[#fafafa]">
                        {row.htf_bullish === 1
                          ? "Bullish"
                          : row.htf_bullish === 0
                            ? "Bearish"
                      ...
                      <td
                        className={`py-2 px-2 font-medium ${row.trend_alignment === "Bullish" ? "text-green-400" : row.trend_alignment === "Bearish" ? "text-red-400" : "text-[#888]"}`}
                      >
                        {row.trend_alignment || "-"}
                      </td>
                    </>
                    ...
                    <>
                      <td
                        className={`py-2 px-2 ${Number(row.delivery_divergence_score) > 0 ? "text-green-400" : "text-red-400"}`}
                      >
                        {row.delivery_divergence_score || "-"}
                      </td>
                      <td className="py-2 px-2 text-[#fafafa]">
                        {row.volatility_compression_score || "-"}
                      </td>
                      <td className="py-2 px-2 text-[#fafafa]">
                        {row.relative_volume_score || "-"}
                      </td>
                      <td
                        className={`py-2 px-2 ${Number(row.nifty_outperformance_score) > 0 ? "text-green-400" : "text-red-400"}`}
                      >
                        {row.nifty_outperformance_score || "-"}
                      </td>
                    </>
                  )}
                </tr>
          ...
          {!dataLoaded && !isRefreshing && (
            <div className="w-full py-8 text-center text-[#666] text-xs font-mono">
              No data loaded or empty result sequence.
            </div>
          )}
        </div>
      </div>
