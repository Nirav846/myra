...
export default function MultibaggerMatrixView({ lib }: { lib: Librarian }) {
  ...
  const fetchMultibaggerCandidates = async () => {
    ...
    try {
      ...
      const fundResult = await lib.executeQuery('_val_conn', fundQuery, {}, 12000);
      ...
      if (fundResult && fundResult.length > 0) {
        ...
        `;
        const priceResult = await lib.executeQuery('_tech_conn', priceQuery, {}, 12000);
        ...
        let mapped = fundResult.filter((d: any) => candidateTickers.includes(`'${d.ticker}'`)).map((d: any) => {
          const accumulation = techMap.get(d.ticker) || 0;
          const closePrice = priceMap.get(d.ticker) || (Number(d.peRatio) * Number(d.earningsPerShare)) || 1;
          const earningsYield = closePrice > 0 ? (Number(d.earningsPerShare) / closePrice) : 0;
          ...
          return {
            ...
            multibagger_index: Math.round((Number(d.returnOnEquity) * 0.4) + (earningsYieldPct * 0.3) + (accumulation * 0.3)),
            moat_score: Number(d.returnOnEquity) > 25 ? 'Monopoly' : Number(d.returnOnEquity) > 15 ? 'High' : 'Medium'
          };
        });
        ...
        if (!lib.isConnectedToLocalRepo) generateMockCandidates();
        else setData([]);
      }
    } catch (e: any) {
      console.error(e);
  ...
  const generateMockCandidates = () => {
    const rawMock = [
      { ticker: 'DIXON', sector: 'Manufacturing', returnOnEquity: 28.4, earningsPerShare: 45.2, peRatio: 85, accumulation: 68.5, moat_score: 'High', bucket: 'Broader Market (N500)' },
      { ticker: 'KPITTECH', sector: 'IT Services', returnOnEquity: 22.1, earningsPerShare: 38.6, peRatio: 65, accumulation: 72.1, moat_score: 'High', bucket: 'Broader Market (N500)' },
      { ticker: 'TRENT', sector: 'Retail', returnOnEquity: 19.5, earningsPerShare: 55.4, peRatio: 120, accumulation: 61.2, moat_score: 'Monopoly', bucket: 'Large Cap (N100)' },
      { ticker: 'POLYCAB', sector: 'Industrials', returnOnEquity: 25.8, earningsPerShare: 28.9, peRatio: 45, accumulation: 58.4, moat_score: 'High', bucket: 'Large Cap (N100)' },
      { ticker: 'HAL', sector: 'Defense', returnOnEquity: 32.4, earningsPerShare: 22.1, peRatio: 35, accumulation: 75.8, moat_score: 'Monopoly', bucket: 'Large Cap (N50)' },
      { ticker: 'ZOMATO', sector: 'Consumer Tech', returnOnEquity: 12.5, earningsPerShare: 6.4, peRatio: 90, accumulation: 54.2, moat_score: 'High', bucket: 'Large Cap (N100)' },
      { ticker: 'CDSL', sector: 'Financials', returnOnEquity: 42.1, earningsPerShare: 28.5, peRatio: 48, accumulation: 66.7, moat_score: 'Monopoly', bucket: 'Broader Market (N500)' },
      { ticker: 'KAYNES', sector: 'Manufacturing', returnOnEquity: 18.2, earningsPerShare: 52.1, peRatio: 105, accumulation: 69.4, moat_score: 'Medium', bucket: 'Broader Market (N500)' },
      { ticker: 'TATASTEEL', sector: 'Metals', returnOnEquity: 14.2, earningsPerShare: 18.5, peRatio: 12, accumulation: 42.1, moat_score: 'Low', bucket: 'Large Cap (N50)' }, 
      { ticker: 'RELIANCE', sector: 'Energy', returnOnEquity: 11.5, earningsPerShare: 14.2, peRatio: 28, accumulation: 50.1, moat_score: 'High', bucket: 'Large Cap (N50)' } 
    ];
    ...
    filtered = filtered.filter(f => f.returnOnEquity >= minRoe && f.earningsPerShare >= minEps);
    if (requireAccumulation) {
    ...
    const calculated = filtered.map(d => {
       ...
       return {
         ...d, 
         earningsYield: earningsYieldPct,
         multibagger_index: Math.round((d.returnOnEquity * 0.4) + (earningsYieldPct * 0.3) + (d.accumulation * 0.3)),
         moat_score: d.returnOnEquity > 25 ? 'Monopoly' : d.returnOnEquity > 15 ? 'High' : 'Medium'
       };
    });
              ...
              <table className="w-full text-left font-mono text-xs relative">
                <thead className="bg-[#1a1c24] border-b border-[#ffffff1a] sticky top-0 z-10">
                  <tr className="text-[#888]">
                    ...
                    <th className="py-3 px-4 font-medium uppercase text-right cursor-pointer hover:text-white" onClick={() => handleSort('returnOnEquity')}>ROE <ArrowUpDown size={10} className="inline ml-1 opacity-50"/></th>
                    <th className="py-3 px-4 font-medium uppercase text-right cursor-pointer hover:text-white" onClick={() => handleSort('earningsYield')}>Earn Yield <ArrowUpDown size={10} className="inline ml-1 opacity-50"/></th>
