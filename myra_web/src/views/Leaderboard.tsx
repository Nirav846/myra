...
export default function LeaderboardView({ lib }: { lib: Librarian }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      try {
        const metaRes = await lib.executeQuery('_val_conn', 'SELECT symbol, COALESCE(market_cap, 0) AS market_cap FROM fundamentals', {}, 5000);
        if (metaRes) {
      ...
      <div className="p-4 space-y-4 relative">
        ...
        <div className={`relative transition-opacity duration-300 ${isRefreshing ? 'opacity-50' : 'opacity-100'}`}>
          <ScrollableTable>
            ...
            <table className="w-full min-w-max whitespace-nowrap text-left font-mono text-xs" aria-label="Leaderboard rankings">
            <thead>
