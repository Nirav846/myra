...
const ChartItem = memo(({ sym, data, toggles, settings }: any) => {
  if (!data)
    return (
      <div
        key={sym}
        className="h-[400px] flex items-center justify-center font-mono text-[#888] text-xs animate-pulse"
      >
        Loading {sym}...
      </div>
    );
...
const ChartItemInner = memo(({ sym, data, toggles, settings }: any) => {
  ...
  const computed = useMemo(() => {
    ...
    const volumes = data.map((d) => {
      const vol =
        d.volume_final != null ? Number(d.volume_final) : Number(d.volume);
    ...
    const deliveryColorsInverse = data.map((d) =>
      d.close >= d.open ? "#ef4444" : "#22c55e",
    );

    // Delivery MA
    const delMaData = toggles.showDelMA
      ? chartRegistry.getIndicator("sma")?.calculate(
          data.map((d) => ({
            ...d,
            close:
              d.delivery_final != null
                ? Number(d.delivery_final)
                : Number(d.delivery_qty) || 0,
          })),
          { period: 20 },
        ) || []
      : [];
    ...
    if (toggles.showSwings) {
      const swingsObj = chartRegistry
        .getIndicator("swings")
        ?.calculate(data, {});
      if (swingsObj) {
        swingsTraces.push(
          ...(chartRegistry
            .getTraceBuilder("swings")
            ?.buildTraces(swingsObj, traceCtx) || []),
        );
      }
    }
    ...
    if (toggles.showVwap) {
      ...
      vwapTraces.push(
        ...(chartRegistry
          .getTraceBuilder("vwap")
          ?.buildTraces(vwapObj, traceCtx) || []),
      );
    }
    ...
    smaConfigs.forEach((cfg) => {
      if (cfg.toggle) {
        const p = chartRegistry
          .getIndicator("sma")
          ?.calculate(data, { period: cfg.period });
        if (p) {
          smaResults[cfg.period] = p;
          smasTraces.push(
            ...(chartRegistry.getTraceBuilder("sma")?.buildTraces(p, traceCtx, {
              period: cfg.period,
              color: cfg.color,
              width: cfg.width,
              yaxis: "y",
    ...
    if (toggles.showRsi) {
      rsiResult =
        chartRegistry.getIndicator("rsi")?.calculate(data, { period: 14 }) ||
        [];
      const tb = chartRegistry.getTraceBuilder("rsi");
      if (tb) {
        rsiTraces.push(
          ...tb.buildTraces(rsiResult, traceCtx, {
            period: 14,
            color: "#8b5cf6",
            width: 1.5,
            yaxis: "y3",
    ...
    if (toggles.showFvg) {
      const activeFVGs =
        chartRegistry
          .getIndicator("fvg")
          ?.calculate(data, { showMitigated: true }) || [];
      const tb = chartRegistry.getTraceBuilder("fvg");
      if (tb && tb.buildShapes) {
        shapes.push(
          ...tb.buildShapes(activeFVGs, traceCtx, { showMitigated: true }),
        );
      }
    }
    ...
    if (toggles.showSmartMoney) {
      const smObj = chartRegistry
        .getIndicator("smartMoneyPrints")
        ?.calculate(data, {});
      if (smObj)
        smartMoneyPrintsTraces.push(
          ...(chartRegistry
            .getTraceBuilder("smartMoneyPrints")
            ?.buildTraces(smObj, traceCtx) || []),
        );
    }

    let delIntensityCoreTraces: any[] = [];
    if (toggles.showDelDivergence) {
      const diObj = chartRegistry
        .getIndicator("delIntensityCore")
        ?.calculate(data, {});
      if (diObj)
        delIntensityCoreTraces.push(
          ...(chartRegistry
            .getTraceBuilder("delIntensityCore")
            ?.buildTraces(diObj, traceCtx) || []),
        );
    }
    ...
    if (toggles.showDeliveryProfile || toggles.showDeliverySR) {
      profileResult = chartRegistry
        .getIndicator("volumeProfile")
        ?.calculate(data, { resolution: toggles.profileResolution }, traceCtx);
      if (profileResult) {
        vpMaxVolume =
          profileResult.volProfileX.length > 0
            ? Math.max(...profileResult.volProfileX)
            : 1;

        const tb = chartRegistry.getTraceBuilder("volumeProfile");
        if (tb) {
          if (toggles.showDeliveryProfile) {
            volProfileTraces.push(
              ...tb.buildTraces(profileResult, traceCtx, {
                resolution: toggles.profileResolution,
                showDeliveryProfile: true,
              }),
            );
          }
          if (tb.buildShapes) {
            shapes.push(
              ...tb.buildShapes(profileResult, traceCtx, {
                resolution: toggles.profileResolution,
    ...
    const { pocVolY: globalPocVolY, pocDelY: globalPocDelY } =
      profileResult || { pocVolY: null, pocDelY: null };

    // 4 Intelligent Indicators
    let instBlocksTraces: any[] = [];
    if (toggles.showInstBlocks) {
      const ibObj = chartRegistry
        .getIndicator("instBlocks")
        ?.calculate(data, {});
      if (ibObj)
        instBlocksTraces.push(
          ...(chartRegistry
            .getTraceBuilder("instBlocks")
            ?.buildTraces(ibObj, traceCtx) || []),
        );
    }
    ...
    if (toggles.showDelVwapBands) {
      ...
      if (dbObj)
        delVwapBandsTraces.push(
          ...(chartRegistry
            .getTraceBuilder("delVwapBands")
            ?.buildTraces(dbObj, traceCtx) || []),
        );
    }
    ...
    if (toggles.showDelAD) {
      ...
      if (daObj)
        delAdTraces.push(
          ...(chartRegistry
            .getTraceBuilder("delAd")
            ?.buildTraces(daObj, traceCtx) || []),
        );
    }
    ...
    if (toggles.showDelivery) {
      ...
      if (tb) {
        deliveryTraces.push(
          ...tb.buildTraces(deliveryFinal, traceCtx, {
    ...
    const paneHeight =
      activePanes > 0
        ? Math.min(
            0.16,
            Math.max(0.05, (0.6 - activePanes * gap) / activePanes),
          )
        : 0;

    const rsiDomain = toggles.showRsi
      ? [currentY, currentY + paneHeight]
      : [0, 0];
    if (toggles.showRsi) currentY += paneHeight + gap;

    const delAdDomain = toggles.showDelAD
      ? [currentY, currentY + paneHeight]
      : [0, 0];
    if (toggles.showDelAD) currentY += paneHeight + gap;

    const delDomain = toggles.showDelivery
      ? [currentY, currentY + paneHeight]
      : [0, 0];
    if (toggles.showDelivery) currentY += paneHeight + gap;

    const volDomain = toggles.showVolume
      ? [currentY, currentY + paneHeight]
      : [0, 0];
    ...
    if (lastIdx >= 0) {
      ...
      if (toggles.showSma20 && smaResults[20])
        pushLabel(smaResults[20][lastIdx], "#ffffff", "rgba(234, 179, 8, 0.8)"); // eab308
      if (toggles.showSma50 && smaResults[50])
        pushLabel(
          smaResults[50][lastIdx],
          "#ffffff",
          "rgba(14, 165, 233, 0.8)",
        ); // 0ea5e9
      if (toggles.showSma150 && smaResults[150])
        pushLabel(
          smaResults[150][lastIdx],
          "#ffffff",
          "rgba(217, 70, 239, 0.8)",
        ); // d946ef
      if (toggles.showSma200 && smaResults[200])
        pushLabel(
          smaResults[200][lastIdx],
          "#ffffff",
          "rgba(249, 115, 22, 0.8)",
        ); // f97316
      if (toggles.showVwap)
        pushLabel(vwap[lastIdx], "#ffffff", "rgba(136, 136, 136, 0.8)");
      ...
      if (toggles.showDelVwapBands && dbObj && dbObj.mid.length > 0) {
        pushLabel(
          dbObj.upper[dbObj.upper.length - 1],
          "#ffffff",
          "rgba(251, 146, 60, 0.5)",
        ); // fb923c
        pushLabel(
          dbObj.lower[dbObj.lower.length - 1],
          "#ffffff",
          "rgba(251, 146, 60, 0.5)",
        );
        pushLabel(
          dbObj.mid[dbObj.mid.length - 1],
          "#000000",
          "rgba(251, 191, 36, 0.8)",
        ); // fbbf24
      }
      ...
      if (toggles.showDeliveryProfile) {
        if (globalPocVolY !== null)
          pushLabel(globalPocVolY, "#ffffff", "rgba(136, 136, 136, 0.8)");
        if (globalPocDelY !== null)
          pushLabel(globalPocDelY, "#ffffff", "rgba(6, 182, 212, 0.8)");
      }
        ...
        data={[
          // Main Candlestick
          {
            ...
            increasing: {
              ...
              fillcolor:
                settings?.candlestickStyle === "Filled" ? "#22c55e" : "#1a1c24",
            }, // hollow body
            decreasing: {
  ...
  return (
    <div
      key={sym}
      className="bg-[#1a1c24] border border-[#ffffff1a] rounded flex flex-col h-[500px] chart-container relative overflow-hidden"
    >
      <ChartHeader
        symbol={sym}
        dataIndex={dataIndex}
      ...
      />
      <div className="flex-1 w-full relative">{plotElement}</div>
    </div>
  );
});
...
export default function AdvancedChartView({
  ...
  const [showSma150, setShowSma150] = usePersistedState(
    "chart-showSma150",
    false,
  );
  const [showSma200, setShowSma200] = usePersistedState(
    "chart-showSma200",
    false,
  );
  const [showFvg, setShowFvg] = usePersistedState("chart-showFvg", true);
  const [showFibonacci, setShowFibonacci] = usePersistedState(
    "chart-showFibonacci",
    false,
  );

  const [showVwap, setShowVwap] = usePersistedState("chart-showVwap", true);
  const [showVolume, setShowVolume] = usePersistedState(
    "chart-showVolume",
    true,
  );
  const [showDelivery, setShowDelivery] = usePersistedState(
    "chart-showDelivery",
    false,
  );
  const [showDelMA, setShowDelMA] = usePersistedState("chart-showDelMA", false);
  const [showDeliveryProfile, setShowDeliveryProfile] = usePersistedState(
    "chart-showDeliveryProfile",
    false,
  );
  const [profileResolution, setProfileResolution] = usePersistedState(
    "chart-profileResolution",
    "auto",
  );
  const [showDeliverySR, setShowDeliverySR] = usePersistedState(
    "chart-showDeliverySR",
    false,
  );
  const [showSmartMoney, setShowSmartMoney] = usePersistedState(
    "chart-showSmartMoney",
    false,
  );
  const [showDelDivergence, setShowDelDivergence] = usePersistedState(
    "chart-showDelDivergence",
    false,
  );

  const [showDelAD, setShowDelAD] = usePersistedState("chart-showDelAD", false);
  const [showDelVwapBands, setShowDelVwapBands] = usePersistedState(
    "chart-showDelVwapBands",
    false,
  );
  const [showLiqVoids, setShowLiqVoids] = usePersistedState(
    "chart-showLiqVoids",
    false,
  );
  const [showInstBlocks, setShowInstBlocks] = usePersistedState(
    "chart-showInstBlocks",
    false,
  );
  ...
  const [showNiftyOut, setShowNiftyOut] = usePersistedState(
    "chart-showNiftyOut",
    false,
  );
  const [showLogScale, setShowLogScale] = usePersistedState(
    "chart-showLogScale",
    false,
  );

  const [showSwings, setShowSwings] = usePersistedState(
    "chart-showSwings",
    true,
  );

  const [scrollEnabled, setScrollEnabled] = usePersistedState(
    "chart-scrollEnabled",
    false,
  );
  ...
  useEffect(() => {
    lib
      ...
      .catch((e) =>
        console.error("Could not fetch all symbols for fast scroll", e),
      );
  }, [lib]);
  ...
  useEffect(() => {
    if (!scrollEnabled || allSymbols.length === 0 || symbols.length === 0)
      return;
  ...
  const sidebarIndicatorToggles = useMemo(
    () => [
      ...
      {
        id: "del_ma",
        label: "Delivery MA (20)",
        state: showDelMA,
        set: setShowDelMA,
      },
      {
        id: "del_profile",
        label: "Vol/Del Profile (FRVP)",
        state: showDeliveryProfile,
        set: setShowDeliveryProfile,
        desc: "Fixed Range Volume Profile (Visible Area). Shows standard Volume (gray) overlaid with Delivery Volume (cyan) and their POCs.",
      },
      {
        id: "del_sr",
        label: "Delivery Auto S/R",
        state: showDeliverySR,
        set: setShowDeliverySR,
        desc: "Auto-draws support/resistance at high delivery price levels.",
      },
      {
        id: "smart_money",
        label: "Smart Money Prints",
        state: showSmartMoney,
        set: setShowSmartMoney,
        desc: "Highlights bars with 1.5x average volume and > 60% delivery ratio.",
      },
      {
        id: "del_divergence",
        label: "Delivery Intensity Core",
        state: showDelDivergence,
        set: setShowDelDivergence,
        desc: "Draws a colored vertical core inside candles representing delivery %. Blue=Institutional, Gold=Divergence, Grey=Retail.",
      },
      {
        id: "del_ad",
        label: "Delivery A/D",
        state: showDelAD,
        set: setShowDelAD,
        desc: "Accumulation/Distribution strictly using delivery volume.",
      },
      {
        id: "del_vwap_bands",
        label: "Del. VWAP Bands",
        state: showDelVwapBands,
        set: setShowDelVwapBands,
        desc: "VWAP weighted by delivery volume with 1.5 standard deviation bands.",
      },
      {
        id: "liq_voids",
        label: "Liquidity Voids",
        state: showLiqVoids,
        set: setShowLiqVoids,
        desc: "Shaded areas where large price movement occurred on low relative volume (potential gap fills).",
      },
      {
        id: "inst_blocks",
        label: "Inst. Blocks",
        state: showInstBlocks,
        set: setShowInstBlocks,
        desc: "Massive volume anomalies (> 3.5x average) paired with > 65% delivery.",
      },
  ...
  return (
    <div
      className="flex bg-[#0e1117] min-h-[600px] border border-[#ffffff1a] rounded overflow-hidden relative"
      ref={containerRef}
    >
      <ChartSidebar
          ...
          <div className="flex items-center gap-3">
            <div className="flex flex-wrap gap-2">
              {symbols.map((sym) => (
                <div
                  key={sym}
                  className="flex items-center gap-1 bg-cyan-500/10 text-cyan-400 text-[10px] px-2 py-1 rounded border border-cyan-500/20 font-mono font-bold"
                >
                  {sym}
                  <button
        ...
        {symbols.length === 0 && (
          <div className="flex-1 flex items-center justify-center flex-col text-[#444]">
            ...
            <p className="font-mono text-sm tracking-tight">
              Search for a ticker to begin analysis
            </p>
          </div>
        )}
      </div>
