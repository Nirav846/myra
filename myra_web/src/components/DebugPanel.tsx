...
export function DebugPanel() {
  ...
  useEffect(() => {
    ...
    const handleDebug = (e: any) => {
      ...
      setEvents((prev) => {
        const newEvents = [
          { ...detail, id: Math.random().toString(36).substr(2, 9) },
          ...prev,
        ];
  ...
  return (
    <div className="fixed bottom-12 right-4 z-50 w-[450px] max-w-[calc(100vw-2rem)] h-[400px] bg-[#0e1117]/95 backdrop-blur-md border border-[#ffffff2a] rounded-lg shadow-2xl flex flex-col font-mono text-xs overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {events.length === 0 ? (
          <div className="text-[#555] text-center mt-10">
            Waiting for queries...
          </div>
        ) : (
          events.map((ev) => (
            <div
              key={ev.id}
              className="bg-[#1a1c24] border border-[#ffffff1a] rounded p-2"
            >
                ...
                <div className="flex justify-between items-center mt-1.5 text-[10px]">
                  {ev.duration !== undefined && (
                    <span className="text-[#888]">
                      Took {ev.duration.toFixed(1)}ms
                    </span>
                  )}
                  {ev.rows !== undefined && (
                    <span className="text-green-400 text-right w-full">
                      Returned {ev.rows} rows
                    </span>
                  )}
                  {ev.error && (
