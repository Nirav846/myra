...
- **Data Layers:** SQLite (Atomic Trilogy Sidecars: `technical.db`, `meta.db`, `institutional.db`, `governance.db`). Thread-safe writes via `with lib._db_lock:` and WAL mode (Rule 43) are strictly enforced.
...
## Architecture
- **Modular Architecture v3.2 (Atomic Trilogy):** A decoupled, sidecar-based system where data (SQL), indicators (Parquet), and logic (Engine) are isolated to ensure stability and performance. SMC FVG threshold is standardized at 1.5%.
