...
```text
RAW SOURCE                      INGESTION                 TRANSFORM/CALCULATE                    STORE                          SERVE
==========                      =========                 ===================                    =====                          =====
NSE Bhavcopy (CSV)       --> fetch_ohlcv_delivery    --> Vectorized Merge & Clean   --> myra_technical.db           --> Screeners
NSE APIs (JSON)          --> GhostSession / fetch_*  --> JSON Parse & Sanitize      --> myra_{inst/gov/meta}.db     --> UI / Dashboards
Screener.in (HTML)       --> fetch_fundamentals      --> BeautifulSoup Extract      --> myra_valuation.db           --> Fundamental Ranker
myra_technical.db        --> precompute_indicators   --> Polars/Pandas Calc         --> Parquet Lake (Indicators)   --> Strategy Engine
```

### C. CRITICAL LOGIC LOCATIONS

*   **Ingestion Pipeline Entry:** `myra_app/daily_ingestor.py` -> `run_daily_update()`
*   **Network Request Engine:** `myra_app/fetcher.py` -> `GhostSession`
*   **Database Definitions:** `myra_app/librarian_schema.py` -> `_create_tables()`
*   **Database Interfacing:** `myra_app/librarian_core.py` -> `safe_execute()`
*   **Indicator Calculation:** `myra_app/librarian_intelligence.py` -> `precompute_indicators()`
*   **Screener Serving Loop:** `myra_app/screener.py` -> `execute_scan()`
*   **Base Strategy Interface:** `myra_app/strategies/base_strategy.py`
