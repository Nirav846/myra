...
def tune_database():
    ...
    console.print(
        "[bold cyan]--- [TUNER] DuckDB Turbo-SQL Optimization ---[/bold cyan]"
    )
    ...
    try:
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_prices_covering ON prices (symbol, date, close, high, low, open, volume, delivery_qty)"
        )
        console.print(
            "[success][✔] Covering index created for 'prices' table.[/success]"
        )
    ...
    try:
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_indicators_covering ON calculated_indicators (date, symbol, sma50, close)"
        )
        console.print(
            "[success][✔] Covering index created for 'calculated_indicators' table.[/success]"
        )
    except Exception as e:
    ...
    try:
        plan = conn.execute(
            "EXPLAIN SELECT symbol, date, close FROM prices WHERE date >= CURRENT_DATE - INTERVAL 756 DAY"
        ).fetchone()[1]
        if "SCAN" in plan:
            console.print(
                "[info][*] Query plan confirmed using efficient Columnar Scan.[/info]"
            )
    except Exception:
        pass
    ...
    console.print(
        f"[success][✔] Optimization complete. DB Size: {round(db_size, 1)} MB[/success]"
    )
