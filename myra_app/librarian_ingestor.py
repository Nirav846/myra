...
class LibrarianIngestorMixin:
    def compact_database(self, conn=None):
        ...
        for db_conn in [self._tech_conn, self._inst_conn, self._meta_conn, self._val_conn]:
            if db_conn:
                try: db_conn.execute("VACUUM")
                except: pass
        if self.conn:
    ...
    def _consolidate_csv_archives(self):
        ...
        if all_data:
            ...
            for f in csv_files: 
                try: os.remove(os.path.join(ad, f))
                except: pass
    ...
    def _fetch_range(self, days, conn=None, existing_dates=None):
        ...
        target_days = [d for d in days if d not in existing_dates] if existing_dates else days
        if not target_days: return True
        ...
        for i in range(0, len(target_days), chunk_size):
            ...
            if self.conn: self.conn.execute("BEGIN TRANSACTION")
            ...
            try:
                for idx, current in enumerate(chunk):
                    ...
                    if not os.path.exists(local):
                        ...
                        if csv_text:
                            with open(local, "w", encoding="utf-8") as f: f.write(csv_text)
                        else: continue
                    
                    if os.path.exists(local):
                        # 1. Ingest into DuckDB (Standard Cache)
                        if self.conn:
                ...
                if self.conn: self.conn.execute("COMMIT")
            except Exception as e: 
                if self.conn: self.conn.execute("ROLLBACK")
                if hasattr(self, 'console'): self.console.print(f"[bold red][!] Ingestion Range Error: {e}[/]")
        return True

    def _ingest_into_sqlite(self, file_path):
        """Helper to ingest a single bhavcopy CSV into technical.db."""
        if not self._tech_conn: return
        try:
            ...
            mapping = {
                ...
                'TTL_TRD_QNTY': 'volume', 'DELIV_QTY': 'delivery',
                'NO_OF_TRADES': 'trades', 'AVG_PRICE': 'vwap', 'DELIV_PER': 'delivery_pct'
            }
            ...
            if hasattr(self, 'console'): self.console.print(f"[bold red][!] SQLite Ingestion Error: {e}[/]")
