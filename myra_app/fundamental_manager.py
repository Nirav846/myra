...
class FundamentalManager:
    ...
    def is_stale(self, symbol, days=90):
        ...
        try:
            ...
            res = conn.execute("SELECT last_fundamental_update FROM symbols_master WHERE symbol = ?", (symbol_clean,)).fetchone()
            conn.close()
            
            if not res or not res[0] or res[0] == '1900-01-01': return True
    ...
    def calculate_f_score(self, symbol):
        ...
        try:
            ...
            df = pd.read_sql("SELECT * FROM quarterly_results WHERE symbol = ? ORDER BY report_date DESC LIMIT 4", conn, params=[symbol_clean])
            conn.close()
    ...
    def get_valuation_metrics(self, symbol):
        ...
        try:
            conn = self._get_val_conn()
            res = conn.execute("SELECT eps, book_value FROM quarterly_results WHERE symbol = ? ORDER BY report_date DESC LIMIT 1", (symbol_clean,)).fetchone()
            conn.close()
