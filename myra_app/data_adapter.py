...
class DataAdapter:
    ...
    def get_price_df(self, symbol: str, lookback_days: int = 252, as_of_date: str = None) -> pd.DataFrame:
        ...
        with self._lock:
            if len(self._price_cache) > 500:
                try: self._price_cache.pop(next(iter(self._price_cache)))
                except: pass
    ...
    def get_latest_funda(self, symbol: str, df: pd.DataFrame = None) -> Dict[str, Any]:
        ...
        if len(funda) <= 1: # Only symbol is present
            ...
            if self.librarian:
                ...
                if os.path.exists(meta_path) and (not funda.get('Sector') or funda.get('Sector') == 'Unknown'):
                    conn_m = sqlite3.connect(meta_path)
                    try:
                        m_res = conn_m.execute("SELECT sector, industry FROM symbols_master WHERE symbol = ?", (symbol_clean,)).fetchone()
                        if m_res:
        ...
        if self.librarian:
            ...
            indicators_df = self.librarian.loader.indicators.load_indicators("precomputed", symbol_clean)
            if not indicators_df.empty:
                ...
                mapping = {
                    'rdv': 'RDV', 'RDV': 'RDV',
                    'money_flow_cr': 'money_flow_cr', 'Money_Flow_Cr': 'money_flow_cr',
                    'smc_phase': 'smc_phase', 'smart_money_score': 'smart_money_score',
                    'sma150': 'sma150', 'sma50': 'sma50', 'sma200': 'sma200'
                }
                for k, v in latest.items():
                    if k in mapping: funda[mapping[k]] = v
                    else: funda[k] = v
    ...
    def get_lookback_for_scanner(self, sid: str) -> int:
        deep_memory = ["109", "110", "111", "127", "126", "bottom_hunter", "whale_tracker", "surpriver_v2", "multibagger_early"]
        if sid in deep_memory: return 756
        return 252
