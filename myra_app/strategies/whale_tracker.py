...
def run(df: pd.DataFrame, funda: dict) -> dict:
    ...
    try:
        ...
        # 2. PERSISTENCE
        committee = None; scaler = None; needs_training = True
        if os.path.exists(model_path):
            try:
                m_data = joblib.load(model_path)
                if (datetime.now() - m_data.get("timestamp", datetime.min)).days < 7:
                    committee = m_data.get("model"); scaler = m_data.get("scaler"); needs_training = False
            except: pass
        ...
        if needs_training:
            ...
            if len(ml_df) < 150 or len(ml_df[ml_df["Target"] == 1]) < 5: return {"signal": False}
            X = ml_df[features]; y = ml_df["Target"]
            scaler = StandardScaler(); X_scaled = scaler.fit_transform(X)
            m1 = xgb.XGBClassifier(n_estimators=30, max_depth=3, learning_rate=0.1, verbosity=0, random_state=42)
            ...
            joblib.dump({"model": committee, "scaler": scaler, "timestamp": datetime.now()}, model_path)
