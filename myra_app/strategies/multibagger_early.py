...
class Strategy:
    ...
    def run(self, df: pd.DataFrame, funda: dict) -> dict:
        ...
        if 'Volume' in last_5.columns: last_5['Volume'] = last_5['Volume'].fillna(0)
        ...
        try:
            ...
            base_range_pct = (base_window['Close'].max() - base_window['Close'].min()) / base_window['Close'].min()
            ...
            # 1. Base & VCP (Max 25)
            if is_tight_base: score += 15
            if is_compressing: score += 10
            
            # 2. Momentum & Trend (Max 25)
            if ema_trend: score += 10
            if rs_raw > 15: score += 15
            elif rs_raw > 0: score += 5
            
            # 3. Institutional Sniper (Max 30)
            if is_vwap_reclaim: score += 15
            rdv = float(funda.get("RDV", 1.0))
            if rdv > 1.5: score += 15
            elif rdv > 1.2: score += 5
            
            # 4. Turnaround Triggers (Max 20)
            if has_rsi_divergence: score += 10
            
            avg_buy = float(funda.get("avg_buy_60d", 0))
            if avg_buy > 0 and ltp < avg_buy: score += 10 # Underwater Bonus

            # --- FINAL GATEKEEPER ---
            # Threshold: 50
            if score < 50:
                return {"signal": False, "reason": "low_probability_setup", "score": score}
            ...
            return {
                ...
                "metrics": {
                    ...
                    "Vibe": "Sniper+VCP" if (is_compressing and is_vwap_reclaim) else "Pre-Expansion",
