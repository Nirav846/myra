...
class WyckoffAutomaton:
    ...
    def _detect_events(
        ...
        for i in range(len(scan_df)):
            ...
            thresholds = self._calculate_dynamic_thresholds(df, symbol, abs_i)
    ...
    def _calculate_dynamic_thresholds(self, df: pd.DataFrame, symbol: str, abs_i: int) -> dict:
        ...
        if len(volume_series) >= 10:
            ...
            if volume_median > 0:
                ...
                thresholds['volume_multiplier'] = 1.8
        else:
            thresholds['volume_multiplier'] = 1.8

        if 'high' in historical_df.columns and 'low' in historical_df.columns and len(historical_df) >= 10:
            hdf = historical_df.copy()
            hdf['tr'] = np.maximum(
                hdf['high'] - hdf['low'],
                np.maximum(
                    abs(hdf['high'] - hdf['close'].shift(1)),
                    abs(hdf['low'] - hdf['close'].shift(1))
                )
            )
            atr = hdf['tr'].rolling(window=min(14, len(hdf)), min_periods=1).mean().iloc[-1]
            avg_price = (hdf['high'].iloc[-1] + hdf['low'].iloc[-1] + hdf['close'].iloc[-1]) / 3
            if avg_price > 0:
                atr_pct = atr / avg_price
                thresholds['price_threshold_pct'] = max(0.1, min(0.5, atr_pct * 2))
            else:
                thresholds['price_threshold_pct'] = 0.35
        else:
            thresholds['price_threshold_pct'] = 0.35
        ...
        if len(delivery_series) >= 10:
            ...
            if delivery_median > 0:
                del_ratio_80th = delivery_80th / delivery_median
                thresholds['delivery_threshold'] = max(20.0, min(60.0, del_ratio_80th * 25.0))
            else:
                thresholds['delivery_threshold'] = 40.0
        else:
            thresholds['delivery_threshold'] = 40.0

        if 'high' in historical_df.columns and 'low' in historical_df.columns and len(historical_df) >= 10:
            hdf2 = historical_df.copy()
            hdf2['range_pct'] = (hdf2['high'] - hdf2['low']) / hdf2['close']
            avg_range_pct = hdf2['range_pct'].mean()
            range_80th = hdf2['range_pct'].quantile(0.8)
            if avg_range_pct > 0:
                range_ratio = range_80th / avg_range_pct
                thresholds['range_multiplier'] = max(1.0, min(1.5, range_ratio * 1.0))
            else:
                thresholds['range_multiplier'] = 1.15
        else:
            thresholds['range_multiplier'] = 1.15

        thresholds['sos_range_pct'] = max(0.3, min(0.6, 0.45 + (thresholds['price_threshold_pct'] - 0.35) * 0.5))
        thresholds['sos_volume_multiplier'] = max(1.0, min(2.0, thresholds['volume_multiplier'] * 0.8))
        thresholds['sos_delivery_multiplier'] = max(0.8, min(1.5, thresholds['delivery_threshold'] / 40.0))
