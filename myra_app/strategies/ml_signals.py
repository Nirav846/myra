...
def run(df: pd.DataFrame, funda: dict) -> dict:
    ...
    """
    if len(df) < 100: return {"signal": False}
    
    try:
        # 1. ADVANCED FEATURE ENGINEERING
        # (Using precomputed metrics from funda where possible for speed)
        df["RSI"] = ta.rsi(df["Close"], length=14)
        df["SMA_Dist"] = (df["Close"] / funda.get("sma50", df["Close"].rolling(50).mean())) - 1
        ...
        labels = []
        # Calculate labels for historical data (vectorized for speed)
        # We only need labels for training rows
        for i in range(len(df) - window):
            current_close = df["Close"].iloc[i]
            future_data = df.iloc[i+1 : i+window+1]
            
            # Find first exit
            hit_pt = (future_data["High"] >= current_close * (1 + pt)).any()
            hit_sl = (future_data["Low"] <= current_close * (1 - sl)).any()
            
            if hit_pt and not hit_sl:
                labels.append(1) # Success
            else:
                labels.append(0) # Fail or Timeout
        ...
        # 3. DATA PREPARATION
        features = ["RSI", "SMA_Dist", "Vol_Shock", "VSA_Spread", "VSA_Closing", "Drawdown", "Log_MF"]
        ml_df = df.dropna(subset=features).copy()
        
        if len(ml_df) < 60: return {"signal": False}
        ...
        if prob_success > 0.70 and is_uptrend:
            return {
                ...
                "metrics": {
                    ...
                    "Vibe": "Institutional Accumulation" if df["Vol_Shock"].iloc[-1] > 1.2 else "Momentum",
