...
class BaseStrategy(ABC):
    ...
    def get_market_mood(self, lib) -> str:
        ...
        try:
            ...
            df_vix = lib.conn.execute("SELECT close FROM benchmark WHERE symbol = '^INDIAVIX' ORDER BY date DESC LIMIT 1").fetchone()
            vix = df_vix[0] if df_vix else 18.0
            
            if vix > 22: return "EXTREME FEAR"
            if vix > 18: return "FEAR"
            if vix < 14: return "GREED"
            return "NEUTRAL"
    ...
    def calculate_kelly_size(self, win_rate: float, reward_to_risk: float) -> float:
        ...
        Formula: K% = W - [(1-W) / R]
        """
        if reward_to_risk <= 0: return 0.05
