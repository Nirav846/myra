...
class Strategy(BaseStrategy):
    ...
    def run(self, df: pd.DataFrame, funda: dict) -> dict:
        ...
        """
        if len(df) < 300: return {"signal": False}
        
        try:
            # 1. Weekly Resample
            df_w = df.resample('W').agg({
                'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum', 'delivery_qty': 'sum'
            }).dropna()
            
            if len(df_w) < 50: return {"signal": False}
            ...
            # 2. Support Levels
            l3y, l2y, l1y = funda.get("low_3y", 0), funda.get("low_2y", 0), funda.get("low_1y", 0)
            ltp = df["Close"].iloc[-1]
            ...
            if not (is_1y or is_2y or is_3y): return {"signal": False}
            ...
            ctx_stage = "Stage 1 (Basing)" if "Stage 1" in stage else "Stage 4 (Dangerous Fall)" if "Stage 4" in stage else stage
            
            # 4. Weekly Divergence
            lookback = df_w.iloc[-26:-2]
            if lookback.empty: return {"signal": False}
            ...
            stars = 1
            if is_divergent: stars += 1
            if is_3y: stars += 1
            if sm_score > 0.7: stars += 1
            if inst_intensity > 0.5: stars += 1
            if mood == "EXTREME FEAR": stars += 1
            
            # 7. SMC: CHoCH
            lookback_choch = df_w.iloc[-15:-3]
            is_choch = latest_w["Close"] > lookback_choch["High"].max() if not lookback_choch.empty else False
            ...
            return {
                "signal": True,
                "stars": "*" * min(5, stars),
                "tactics": {"entry": entry, "sl": sl, "target": round(entry + (5 * (entry-sl)), 2)},
                "metrics": {
                    "Support": f"{'3Y' if is_3y else '2Y' if is_2y else '1Y'} Floor",
                    "Weekly_Div": "YES" if is_divergent else "NO",
                    "CHoCH": "YES" if is_choch else "NO",
                    "A/D_Vibe": "Accumulating" if funda.get("AD_Flow", 0) > 0 else "Distributing",
