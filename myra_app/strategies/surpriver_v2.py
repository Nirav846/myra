    ...
    def run(self, df: pd.DataFrame, funda: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for w in self.windows:
                acc_days = (df['daily_acc_score'] > 0.3).rolling(w).sum() # Lowered to 0.3
            ...
            df['buying_wick'] = (df['Close'] > df['Open']) & (df['Low'] < df['Low'].shift(1).fillna(df['Low']))
            absorption_consistency = 0
            for w in self.windows:
                if (df['buying_wick'].rolling(w).sum() >= w * 0.3).iloc[-1]: # Lowered to 30%
