...
class BhavcopyParser:
    ...
    def detect_file_format(filename: str) -> str:
        ...
        if re.match(r"nse_full_\d{4}-\d{2}-\d{2}\.csv", filename):
            return "YYYY-MM-DD"
        elif re.match(r"nse_full_\d{8}\.csv", filename) or re.match(r"bhavcopy\d{8}\.csv", filename):
            return "DDMMYYYY"
        return "UNKNOWN"
        ...
        try:
            ...
            if isinstance(data, str):
                if "\n" in data:
                    ...
                    report["errors"].append(f"String data provided is neither valid CSV nor a valid filepath.")
                    return pd.DataFrame(), report
            else:
            ...
            if df.empty:
                report["errors"].append("DataFrame is empty after load (possibly only headers).")
                return pd.DataFrame(), report
            ...
            df.columns = [SchemaRegistry.get_canonical_column(col) for col in df.columns]

            # SCHEMA VALIDATION LAYER (PRIORITY 1.3)
            required_cols = SchemaRegistry.TABLES["technical_data"]["required_for_ingestion"]
            ...
            if missing_cols:
                ...
                if "date" in missing_cols and source_filename:
                    ...
                    if extracted_date:
                        ...
                        logger.warning(f"Recovered missing 'date' column from filename {source_filename}.")
            ...
            numeric_cols = ["open", "high", "low", "close", "volume", "delivery", "delivery_pct", "trades", "vwap"]
            ...
            if "delivery" in df.columns and "volume" in df.columns:
                 ...
                 if "delivery_pct" not in df.columns:
                     df.loc[mask, "delivery_pct"] = (df.loc[mask, "delivery"] / df.loc[mask, "volume"]) * 100
                 if "delivery_ratio" not in df.columns:
                     df.loc[mask, "delivery_ratio"] = df.loc[mask, "delivery"] / df.loc[mask, "volume"]
