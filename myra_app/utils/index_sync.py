...
def sync_index_constituents(index_name, force=False):
    ...
    try:
            ...
            if last_sync:
                try:
                    ...
                    if days_since_sync < 15:
                        print(f"[Index Sync] {index_name} synced {days_since_sync} days ago. Skipping.")
                        return True
                except ValueError:
                    pass
        ...
        with sqlite3.connect(metadata_db_path, timeout=30) as conn:
            ...
            # Clear old entries
            conn.execute("DELETE FROM index_constituents WHERE index_name = ?", (index_name,))
        ...
        print(f"[Index Sync] Successfully synced {len(symbols)} symbols for {index_name}")
        return True
