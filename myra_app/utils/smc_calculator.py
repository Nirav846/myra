...
def calculate_smc_indicators(df: pd.DataFrame, swing_length: int = 5) -> pd.DataFrame:
    ...
    roll_high = df.groupby("symbol")["High"].rolling(
        2 * n + 1, center=True, min_periods=1
    )
    roll_low = df.groupby("symbol")["Low"].rolling(
        2 * n + 1, center=True, min_periods=1
    )

    df["swing_high"] = (
        df["High"] == roll_high.max().reset_index(level=0, drop=True)
    ).astype(float) * df["High"]
    df["swing_low"] = (
        df["Low"] == roll_low.min().reset_index(level=0, drop=True)
    ).astype(float) * df["Low"]
    ...
    df["trend_alignment"] = (
        df["htf_bullish"] + df["mtf_bullish"] - df["htf_bearish"] - df["mtf_bearish"]
    )

    # --- Delivery MA ---
    deliv_col = "delivery_qty" if "delivery_qty" in df.columns else "delivery"
    df["delivery_ma_60"] = df.groupby("symbol")[deliv_col].transform(
        lambda x: x.rolling(60).mean()
    )

    # --- Active bullish FVG ---
    df["has_bullish_fvg"] = (
        (df["bullish_fvg"] == 1) & (df["fvg_freshness"] <= 10)
    ).astype(int)
