...
def calculate_smc_indicators(
    ...
    df_pl = df_pl.with_columns([
        (pl.col("low") > pl.col("high").shift(2).over("symbol")).alias("bullish_fvg"),
        (pl.col("high") < pl.col("low").shift(2).over("symbol")).alias("bearish_fvg")
    ])
    
    # --- FVG boundaries ---
    df_pl = df_pl.with_columns([
        pl.when(pl.col("bullish_fvg")).then(pl.col("low"))
        .when(pl.col("bearish_fvg")).then(pl.col("high").shift(2).over("symbol"))
        .otherwise(None).alias("fvg_top"),
        pl.when(pl.col("bullish_fvg")).then(pl.col("high").shift(2).over("symbol"))
        .when(pl.col("bearish_fvg")).then(pl.col("low"))
        .otherwise(None).alias("fvg_bottom"),
    ])
    
    df_pl = df_pl.with_columns([
        pl.when(pl.col("close") > pl.col("fvg_top")).then(pl.col("fvg_top"))
        .when(pl.col("close") < pl.col("fvg_bottom")).then(pl.col("fvg_bottom"))
    ...
    df_pl = df_pl.with_columns([
        pl.when(pl.col("fvg_event")).then(pl.col("row_idx")).otherwise(None)
        .over("symbol").forward_fill().alias("last_event_idx")
    ])
    df_pl = df_pl.with_columns([
        (pl.col("row_idx") - pl.col("last_event_idx")).fill_null(-1000).alias("fvg_freshness")
    ])
    ...
    df_pl = df_pl.with_columns([
        pl.col("high").rolling_max(window_size=window, center=True).over("symbol").alias("rolling_high_max"),
        pl.col("low").rolling_min(window_size=window, center=True).over("symbol").alias("rolling_low_min")
    ])
    df_pl = df_pl.with_columns([
        pl.when(pl.col("high") == pl.col("rolling_high_max")).then(pl.col("high")).otherwise(None).alias("swing_high"),
        pl.when(pl.col("low") == pl.col("rolling_low_min")).then(pl.col("low")).otherwise(None).alias("swing_low")
    ])
    ...
    df_pl = df_pl.with_columns([
        (pl.col("htf_bullish") + pl.col("mtf_bullish") - pl.col("htf_bearish") - pl.col("mtf_bearish")).alias("trend_alignment")
    ])
    
    # --- Delivery MA ---
    if "delivery_qty" in df_pl.columns:
        df_pl = df_pl.with_columns([
            pl.col("delivery_qty").rolling_mean(window_size=60).over("symbol").alias("delivery_ma_60")
        ])
    elif "delivery" in df_pl.columns:
        df_pl = df_pl.with_columns([
            pl.col("delivery").rolling_mean(window_size=60).over("symbol").alias("delivery_ma_60")
        ])
    else:
    ...
    # --- Active bullish FVG ---
    df_pl = df_pl.with_columns([
        (pl.col("bullish_fvg") & (pl.col("fvg_freshness") <= 10)).cast(pl.Int32).alias("has_bullish_fvg")
    ])
    
    # Convert boolean to int for consistency
    df_pl = df_pl.with_columns([
        pl.col("bullish_fvg").cast(pl.Int32),
        pl.col("bearish_fvg").cast(pl.Int32)
    ])
