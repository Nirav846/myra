        ...
        with sqlite3.connect(_DB_PATH, timeout=10) as conn:
            ...
            if not force:
                ...
                if last:
                    ...
                    if days_since < 6:
                        logger.info(
                            f"ETF sync skipped — last sync {days_since} days ago"
                        )
                        return False
...
def purge_etf_rows_from_technical_db(dry_run: bool = False) -> int:
    ...
    try:
        with sqlite3.connect(tech_db, timeout=30) as conn:
            ...
            # Vacuum if we removed a significant number of rows
            if count > 10000:
                print(f"  [INFO] Vacuuming database after removing {count} ETF rows...")
                conn.execute("VACUUM")
