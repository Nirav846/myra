...
class ResultsManager:
    ...
    def display_last_results(self):
        ...
        if not os.path.exists(path):
            self.console.print("[warning][!] No previous scan results found in cache.[/warning]")
            return
        ...
        try:
            ...
            std = ["Stock", "Stars", "Grade", "MYRA_Score_v25", "Stage", "LTP", "Entry", "TSL", "Vibe", "FVG_Zone", "CHoCH", "Support"]
    ...
    def display_portfolio_risk_audit(self, results: list):
        ...
        t_sector = Table(title="Sector Exposure Audit", header_style="bold cyan", border_style="dim")
        t_sector.add_column("Sector"); t_sector.add_column("Count"); t_sector.add_column("Concentration")
        total = len(results)
        ...
        t_risk = Table(title="Portfolio Risk Metrics", header_style="bold red", border_style="dim")
        t_risk.add_column("Stock"); t_risk.add_column("Max Drawdown"); t_risk.add_column("Volatility (Ann)"); t_risk.add_column("Risk Rating")
        
        for r in results:
            mdd = r.get("MDD", 0)
            vol = r.get("Vol_Ann", 0)
            rating = "[green]LOW[/green]" if vol < 20 else "[yellow]MOD[/yellow]" if vol < 40 else "[red]HIGH[/red]"
    ...
    def save_charts(self, results: list, lib):
        ...
        for r in results[:5]:
            try:
                ...
                plt.plot(df.index, df['Close'], color='white', label='Price', linewidth=2)
                plt.plot(df.index, df['Close'].rolling(20).mean(), color='cyan', label='SMA 20', alpha=0.8)
                plt.plot(df.index, df['Close'].rolling(50).mean(), color='yellow', label='SMA 50', alpha=0.8)
                plt.title(f"MYRA Intelligence: {symbol}", color='magenta', fontsize=14)
                plt.legend(); plt.grid(color='#333', linestyle='--', alpha=0.5)
                plt.savefig(os.path.join(self.chart_dir, f"{symbol}_chart.png"))
                plt.close()
            except Exception: pass

    def display_discovery_table(self, results: list, scan_name: str, strategy_id: str, hero_cols: list, strictly_technical: bool = False):
        ...
        table.add_column("Stock", style="bold yellow", overflow="ellipsis", no_wrap=True, min_width=8, header_style="bold yellow")
        if show_sector:
            table.add_column("Sector", style="dim cyan", no_wrap=True, header_style="bold yellow")
        table.add_column("Rating", justify="center", header_style="bold yellow")
        
        if not strictly_technical:
            if not is_mobile: table.add_column("Grade", justify="center", header_style="bold yellow")
            if has_v25:
                table.add_column("Score", justify="right", style="bold green", header_style="bold yellow")
            
            # Technical DNA Group (Cyan)
            if not is_narrow:
                table.add_column("Accuracy", justify="right", header_style="cyan")
                table.add_column("Pattern", justify="left", overflow="fold", header_style="cyan")
            
            if not is_mobile: table.add_column("Money_Flow", justify="right", style="bold cyan", header_style="cyan")
            
            table.add_column("Stage", style="bold magenta", justify="center", header_style="cyan")
            table.add_column("LTP", justify="right", header_style="cyan")
            
            # Tactical Zone (Green)
            mid_col = "FVG Zone" if strategy_id == "bottom_hunter" else "Best Buy"
            table.add_column(mid_col, style="green", justify="center", header_style="bold green")
            
            if not is_narrow: table.add_column("Vibe", justify="center", header_style="bold green")
        
        # Institutional Alpha Group (Magenta)
        for c in hero_cols:
            # Shorten column headers for UI density
            header = c.replace("Anomaly_Score", "Anom").replace("Active_Windows", "Win").replace("Absorp_Ratio", "Absorp").replace("Absorption", "Absorp")
            if header == "Stage": header = "Trend" # Avoid collision
            table.add_column(header, justify="right", style="cyan", header_style="bold magenta")
        ...
        for idx, r in enumerate(results):
            ...
            ltp = r.get("LTP", "-")
            if isinstance(ltp, (float, int, np.number)): ltp = f"{float(ltp):.2f}"
            ...
            if strategy_id == "bottom_hunter":
                ...
                val = f"{float(tsl_val):.2f}" if isinstance(tsl_val, (float, int, np.number)) else str(tsl_val)
            else:
                # Prioritize FVG Midpoint for Best Buy
                val = f"{float(entry_val):.2f}" if isinstance(entry_val, (float, int, np.number)) else str(entry_val)
            ...
            if is_narrow: s_str = s_str.replace("Stage ", "S") # Compact Stage
            ...
            for c in hero_cols:
                ...
                try:
                    if raw_val is None or str(raw_val).lower() == "nan" or raw_val == "-":
                        formatted_val = "-"
                    elif isinstance(raw_val, (float, np.float64, np.float32, int, np.int64)):
                        formatted_val = f"{float(raw_val):.2f}"
                    elif isinstance(raw_val, str):
                        # Try to parse float from string if it's not a special text
                        try:
                            num = float(raw_val.replace("%", "").replace("x", "").replace("₹", "").replace("Cr", "").replace(",", ""))
                ...
                try:
                    ...
                    clean_str = formatted_val.replace("%", "").replace("x", "").replace("₹", "").replace("Cr", "").replace(",", "")
                    ...
                    if c == "AEON_Conviction":
                        if "CONVICTION" in formatted_val: colored_val = f"[bold green]{formatted_val}[/]"
                        elif "CORE" in formatted_val: colored_val = f"[bold cyan]{formatted_val}[/]"
        ...
        total = len(results)
        strong = len([r for r in results if "2" in str(r.get("Stage", ""))])
        self.console.print(f"[dim]  [+] Scan Complete: {total} candidates identified. Strong Trend (Stage 2): {strong} [/dim]")
    ...
    def offer_institutional_deep_dive(self, results: list):
        ...
        self.console.print(rf"\n[bold cyan][?][/bold cyan] [white]Found {len(results)} candidates. Run Institutional Deep-Dive (DCF + Red Flags) on top {top_n}? [y/N][/white] ", end="")
    ...
    def run_institutional_deep_dive(self, results: list, db_conn, fetcher):
        ...
        with self.console.status("[bold green]Fetching Deep History & Computing Intrinsic Alpha...[/bold green]"):
        ...
        t = Table(title="Institutional Deep-Dive (TRILOGY-DFR)", header_style="bold magenta", border_style="dim", box=None)
        ...
        for sym, res in deep_results.items():
            ...
            flags = "\n".join([f"• {f}" for f in res.get("flags", [])]) if res.get("flags") else "[dim]No institutional flags detected[/dim]"
        ...
        self.console.print("[dim][*] Valuation logic: Gordon Growth Model (DCF) with 5-year CAGR decay and sector-adjusted WACC.[/dim]")

    def display_execution_plan(self, results: list, scan_name: str):
        if not results: return
        t = Table(title=f"Tactical Execution Plan: {scan_name}", header_style="bold red", border_style="dim", style="on black")
        t.add_column("Stock", style="bold yellow")
        ...
        t.add_column("VaR (95%)", justify="right") # Value at Risk (PKScreener Superpower)
        ...
        for r in results:
            ...
            try:
                ...
                kelly_pct = KellySizer().get_allocation_pct(win_rate, 3.0, 1.0) # Using 3:1 RR default
                
                # 2. Risk Parity Quantity (Volatility Adj)
                # Assuming standard 10L capital for display purposes
                qty = VolatilityAdjustedSizer().get_quantity(entry, atr, 1000000, risk_per_trade_pct=1.0)
