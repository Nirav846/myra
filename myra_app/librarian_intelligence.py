        ...
        for sym in tqdm(active_symbols, desc="Precomputing"):
            try:
                ...
                if 'Delivery_qty' in df.columns:
                    ...
                    df['money_flow_cr'] = (df['Delivery_qty'] * df['Close']) / 10000000.0
                else:
                    df['rdv'] = 1.0; df['money_flow_cr'] = 0.0
                ...
                df['smc_phase'] = 0
                cond2 = (df['Close'] > df['d_poc'] * 1.03) & (df['Close'] >= df['high_1y'].shift(1)) & (df['Volume'] > df['avg_volume_20d'] * 1.5)
                df.loc[cond2, 'smc_phase'] = 2
                
                # 6. SMC-2: Market Structure (CHoCH, BOS, FVG)
                from myra_app.engine import SMCManager
                # FVG
                df['fvg'] = SMCManager.calculate_fvg(df.rename(columns=lambda x: x.lower()))
                
                # BOS & CHoCH
                bos, choch = SMCManager.calculate_market_structure(df.rename(columns=lambda x: x.lower()))
                df['bos'] = bos
                df['choch'] = choch
