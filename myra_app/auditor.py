...
class TrustLoopAuditor:
    ...
    def run_audit(self):
        ...
        self.console.print("[info][*] Trust Loop: Auditing institutional signal performance...[/info]")
        ...
        if pending.empty:
            self.console.print("[dim][*] Trust Loop: No pending signals for audit.[/dim]")
            return
        ...
        if updated_count > 0:
            self.console.print(f"[success][*] Trust Loop: Updated {updated_count} signals with latest performance metrics.[/success]")
    ...
    def _audit_one_signal(self, signal):
        ...
        risk = signal_price - sl_price if signal_price > sl_price else (signal_price * 0.05)
        ...
        grade = "C"
        if latest_price <= sl_price: grade = "F"
        elif gain_pct >= 0.15 or rr_ratio >= 3.0: grade = "A"
        elif gain_pct >= 0.07 or rr_ratio >= 1.5: grade = "B"
        elif gain_pct < 0: grade = "F"
        ...
        """
        try:
            self.lib.safe_execute(update_sql, (
                mfe, mae, price_t10, price_t20, price_t30, 
                rr_ratio, grade, is_matured,
