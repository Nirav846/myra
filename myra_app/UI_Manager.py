...
class MYRA_UI:
    ...
    def get_header(librarian, forecast=None):
        ...
        try:
            ...
            if nifty and vix:
                ...
                v_col = "red" if vix['last_price'] > 18 else "green" if vix['last_price'] < 15 else "yellow"
    ...
    def get_menu_grid():
        ...
        tech_panel = MYRA_UI.get_category_panel("Technical Core", [
            "1 > Classical Scout", "2 > Deliv Spikes", 
            "3 > RS Rating", "4 > BB Squeeze", "5 > MACD Momentum", 
            "6 > Breakouts", "7 > Reversals", "9 > VSA Momentum", 
            "28 > RS Momentum", "29 > Morning Fakeout"
        ], "yellow")

        # 2. Alpha Intelligence (Magenta)
        alpha_panel = MYRA_UI.get_category_panel("Alpha Intelligence", [
            "A1 > VCP Base Breakout", "A2 > Weekly Bear Trap", 
            "A3 > RS Leadership (S2)", "A4 > Earnings Drift", 
            "A5 > Delivery Clusters", "A6 > Stage 2 Cont.", 
            "A0 > Alpha Ranker (IAS)", "30 > Smart Money Ignition",
            "33 > Structural Flow (SMC-2)", "35 > Multibagger Early"
        ], "magenta")

        # 3. AI & Quant Engine (Cyan)
        ml_panel = MYRA_UI.get_category_panel("AI & Quant Engine", [
            "14 > ML-Signals", "15 > Elite Whale Tracker", 
            "31 > AEON Agent (ML-1)", "32 > Dilated CNN (ML-2)", 
            "20 > Intelligence Scout", "24 > Crash Resilience", 
            "19 > Market Breadth", "M  > Watchdog Daemon", 
            "L  > Last Results"
        ], "cyan")

        # 4. Strategic Command (Green)
        val_panel = MYRA_UI.get_category_panel("Strategic Command", [
            "0 > Daily Routine", "12 > Super-Scan (RS)", 
            "27 > Bottom Hunter", "8  > Value (ROE/PEG)", 
            "13 > Deep Value (Graham)", "21 > Portfolio Monitor", 
            "26 > Piped Playbook", "T  > IAS Timing Engine Picks",
            "Z  > Exit System"
        ], "green")
    ...
    def get_ias_leaderboard(librarian):
        ...
        try:
            if not librarian or not hasattr(librarian, '_gov_conn') or not librarian._gov_conn:
                ...
                if os.path.exists(gov_db):
                    ...
                    return Panel("[red]Governance DB Missing[/]", title="IAS Leaderboard")
            else:
            ...
            table = Table(title="[bold magenta]IAS Leaders (Institutional)[/]", expand=True, header_style="bold magenta", box=None)
        ...
        try:
            if not librarian or not hasattr(librarian, '_gov_conn') or not librarian._gov_conn:
                ...
                if os.path.exists(gov_db):
                    ...
                    return Panel("[red]Governance DB Missing[/]", title="Timing Triggers")
            else:
                conn = librarian._gov_conn
            ...
            table = Table(title="[bold green]Timing Triggers (IAS >= 7)[/]", expand=True, header_style="bold green", box=None)
            table.add_column("Symbol", style="bold yellow")
            ...
            for symbol in df_ias['symbol']:
                ...
                if res and res.get('signal'):
                    m = res.get('metrics', {})
                    table.add_row(symbol, str(m.get('Type', '-')), str(m.get('Entry', '-')))
                    count += 1
                    if count >= 5: break
            ...
            return Panel(f"[dim]Timing Feed Error: {str(e)}[/]", title="Timing Triggers")

    @staticmethod
    def get_footer(librarian, market_breadth="↗ 0 | ↘ 0", forecast=None):
        db_stats = librarian.get_db_stats() if librarian else {"status": "Connected", "size": "0MB"}
