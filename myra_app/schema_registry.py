...
class SchemaRegistry:
    ...
    COLUMN_MAPPINGS = {
        ...
        "volume": ["TTL_TRD_QNTY", "TOTTRDQTY", "VOLUME", "Volume", "volume", "TRADED_QTY"],
        "delivery": ["DELIV_QTY", "DELIVERY_QTY", "Delivery Quantity", "delivery", "Deliverable Volume", "DELIVERABLE_VOLUME"],
        "delivery_pct": ["DELIV_PER", "DELIVERY_PCT", "Delivery Percentage", "delivery_pct", "% Dly Qt to Traded Qty", "DELIVERY_TO_TRADED_QUANTITY"],
    ...
    def get_canonical_column(cls, raw_header: str) -> str:
        ...
        for canonical_key, variations in cls.COLUMN_MAPPINGS.items():
            if raw_clean in variations or raw_clean.upper() in [v.upper() for v in variations]:
