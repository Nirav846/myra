...
warnings.filterwarnings("ignore", message=".*deprecated now, and have no effect.*", category=DeprecationWarning)
...
def _worker_task(payload):
    ...
    if _worker_adapter is None: return None
...
class Engine:
    ...
    def run_scan(self, symbols: List[str], strategy_name: str, as_of_date: str = None, silent: bool = False):
        ...
        if fetcher._is_holiday(target_date):
            if not silent: print(f"[MYRA] {target_date} is a Market Holiday. Attempting Snapshot...")
            try:
                from myra_app.results_manager import ResultsManager
                rm = ResultsManager()
                snapshot = rm.load_last_snapshot(strategy_name)
                if snapshot:
                    if not silent: print(f"[MYRA] Success: Loaded Last Known Good Snapshot.")
                    return snapshot, {"status": "HOLIDAY_SNAPSHOT"}
            except Exception: pass
            
            if not as_of_date:
                last_trading_day = lib.get_expected_trading_day(datetime.now())
                if not silent: print(f"[MYRA] No Snapshot found. Targeting last trading day: {last_trading_day}")
        ...
        try:
            ...
            for c in cache_records:
                ...
                i = insider_map.get(s, {"buy_latest": 0, "total_60d": 0, "avg_buy_60d": 0, "accel": 0})
                ...
                funda_map[s] = {
                    ...
                    "ProfitGrowth": f.get('profit_growth', 0) or 0, "SalesGrowth": f.get('sales_growth', 0) or 0,
                    ...
                    "rel_spread": rel_spread, "rel_vol": rel_vol,
            ...
            payloads = [(s, strategy_name, as_of_date, funda_map.get(s, {"symbol": s})) for s in target_symbols]
        ...
        try:
            ...
            with multiprocessing.Pool(processes=max_workers, initializer=init_worker, initargs=(strategy_name, lib.db_path)) as pool:
                ...
                results = [res for res in tqdm(poked_results(raw_it, watchdog), total=num_stocks, desc="Scanning", unit="stock", leave=False, disable=silent) if res]
            ...
            if not silent: print(f"[!] Engine Error: {e}")
        finally:
            watchdog.stop()
