...
class IASManager:
    ...
    def calculate_ias(self, symbol, df=None):
        ...
        if sast_score >= 8 and del_score >= 8: bonus += 1.0
        # Trap Detection: Bear Trap + High Delivery
        if self._detect_bear_trap(df) and del_score >= 8: bonus += 1.0
        # Ready: High Compression + Near Resistance
        if comp_score >= 8 and self._is_near_resistance(df): bonus += 0.5

        final_ias = min(base_ias + bonus, 10.0)
        
        # Classification
        tag = "NO_EDGE"
        if final_ias >= 8: tag = "STRONG_ACCUMULATION"
        elif final_ias >= 7: tag = "EARLY_ACCUMULATION"
        elif final_ias >= 5: tag = "WATCHLIST"

        return round(final_ias, 2), tag

    def _get_sast_score(self, symbol):
        """Analyzes SAST disclosures from governance.db."""
        if not os.path.exists(self.gov_db): return 4.0, {}
        ...
        try:
            ...
            for r_type, qty in rows:
                val = float(qty or 0)
                if r_type == 'BUY': net_pct += val
                else: net_pct -= val
                if val > max_single: max_single = val
            
            score = 4.0
            if net_pct >= 0.5: score = 10.0
            elif net_pct >= 0.2: score = 8.0
            elif net_pct > 0: score = 6.0
            elif net_pct < 0: score = 2.0
            
            if max_single > 0.5: score = min(score + 2.0, 10.0)
            if len(rows) > 1: score = min(score + 1.0, 10.0) # Cluster bonus
    ...
    def _get_delivery_score(self, df):
        try:
            # Need delivery_pct
            d_pct = df['delivery_pct'].iloc[-20:] if 'delivery_pct' in df.columns else pd.Series([0]*20)
            avg_5d = d_pct.iloc[-5:].mean()
            
            score = 3
            if avg_5d > 55: score = 10
            elif avg_5d > 45: score = 8
            elif avg_5d > 35: score = 6
            
            # Cluster Bonus: 3 spikes in 5 days (spike = > 1.5x 20d avg)
            avg_20d = d_pct.mean()
            spikes = (d_pct.iloc[-5:] > (avg_20d * 1.5)).sum()
            if spikes >= 3: score = min(score + 2, 10)
    ...
    def _get_price_structure_score(self, df):
        try:
            ...
            if range_10d < 0.05 and hl: return 9
            if range_10d < 0.05: return 7
            
            # Near 52w low (30% buffer)
            low_52w = df['Low'].min()
            if c.iloc[-1] < (low_52w * 1.3): return 6
    ...
    def _get_volume_pattern_score(self, df):
        try:
            ...
            if dry_up: return 10 # Anticipating breakout
            if declining: return 7
            return 5
        except: return 3
    ...
    def sync_sast_incremental(self):
        ...
        count = 0
        for d in data:
            # Create unique ID: symbol_date_acq_qty
            raw_id = f"{d.get('symbol')}_{d.get('date')}_{d.get('acqName')}_{d.get('secAcq')}"
            uid = hashlib.md5(raw_id.encode()).hexdigest()
            
            try:
                conn.execute("""
                    INSERT OR IGNORE INTO sast_disclosures 
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (uid, d.get('symbol'), d.get('date'), d.get('acqName'), 
    ...
    def sync_pledge_full(self, symbols):
        ...
        for s in symbols:
            ...
            if data:
                # Typically data is a list of rows, we want the aggregate/latest
                try:
                    # Logic to sum or take latest row from NSE JSON
                    latest = data[0]
                    conn.execute("""
                        INSERT OR REPLACE INTO pledged_history (symbol, date, promoter_holding, pledged_pct)
                        VALUES (?, ?, ?, ?)
                    """, (s, now, self.fetcher.sanitize_float(latest.get('promoterHolding')), 
    ...
    def run_governance_audit(self, symbol):
        ...
        flags = []
        if not os.path.exists(self.gov_db): return flags
        ...
        try:
            ...
            if sell_count >= 3:
                flags.append(f"[yellow]WARNING: {sell_count} Insider Sell Disclosures in 30d[/]")
