...
class DataFetcher:
    ...
    def fetch_insider_trades(self, days=30):
        ...
        params = {
            "index": "equities",
            "from_date": fmt_dmy(start_date),
            "to_date": fmt_dmy(end_date),
        }
        bse_params = {"strpdt": fmt_ymd(start_date), "strfdt": fmt_ymd(end_date)}
        streams = [
            {
                "name": "nse_primary",
            ...
            for attempt in range(3):
                try:
                    if "nse" in stream["name"]:
                        self.session.get(
                            "https://www.nseindia.com", headers=headers, timeout=10
                        )
                        r = self.session.get(
                            stream["url"], params=params, headers=headers, timeout=15
                        )
                        if getattr(r, "status_code", 0) == 200:
                            try:
                                ...
                                raise ValueError(
                                    f"Invalid JSON Structure from {stream['name']}"
                                )
                            ...
                            if data is not None:
                                ...
                                if len(data) == 0:
                                    raise ValueError(
                                        f"Received structurally valid but empty dataset from {stream['name']}"
                                    )
                                ...
                                for d in data:
                                    if isinstance(d, dict):
                                        ...
                                        if "secVal" not in d and "secAcq" not in d:
                                            raise ValueError(
                                                "Missing critical fields (secVal/secAcq) in NSE payload"
                                            )

                                        d["secVal"] = self.sanitize_float(
                                            d.get("secVal", 0)
                                        )
                                        d["secAcq"] = self.sanitize_float(
                                            d.get("secAcq", 0)
                                        )
                                        ...
                                        if d_date:
                                            try:
                                                ...
                                                pd_date = datetime.strptime(
                                                    d_date[:11].strip(), "%d-%b-%Y"
                                                )
                                                if (
                                                    latest_date is None
                                                    or pd_date > latest_date
                                                ):
                                                    latest_date = pd_date
                                            except:
                                                pass
                                ...
                                if latest_date:
                                    if (datetime.now() - latest_date).days > 14:
                                        logger.warning(
                                            f"Data from {stream['name']} is stale (latest record > 14 days old)."
                                        )
                        ...
                        with urllib.request.urlopen(req, timeout=15) as response:
                            if response.status == 200:
                                try:
                                    ...
                                    raise ValueError(
                                        f"Failed parsing BSE fallback response: {json_e}"
                                    )

                                data = (
                                    json_payload.get("Table", [])
                                ...
                                if data is not None:
                                    ...
                                    for d in data:
                                        if isinstance(d, dict):
                                            # Data Validation 2: Key Fields Presence
                                            if (
                                                "ValueofSecurityAcq" not in d
                                                and "NoOfSecuritiesAcq" not in d
                                            ):
                                                raise ValueError(
                                                    "Missing critical fields (secVal/secAcq equivalents) in BSE payload"
                                                )

                                            d["secVal"] = self.sanitize_float(
                                                d.get("ValueofSecurityAcq", 0)
                                            )
                                            d["secAcq"] = self.sanitize_float(
                                                d.get("NoOfSecuritiesAcq", 0)
                                            )
        ...
        logger.error(
            "All insider trading fetch attempts failed (Primary and Fallbacks)."
        )
        raise RuntimeError(
            "CRITICAL: Failed to fetch insider trades from all configured sources. Halting pipeline to prevent silent data corruption."
        )
