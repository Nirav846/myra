...
class PatternScout:
    ...
    def get_latest_pattern(self, df: pd.DataFrame) -> str:
        ...
        try:
            ...
            patterns = df.ta.cdl_pattern(name=["hammer", "engulfing", "morningstar", "doji", "shootingstar", "invertedhammer", "marubozu"])
            if patterns is not None and not patterns.empty:
                ...
                if not active.empty:
                    ...
                    val = active.iloc[0]
                    return f"[green]Bullish {p_name}[/green]" if val > 0 else f"[red]Bearish {p_name}[/red]"
