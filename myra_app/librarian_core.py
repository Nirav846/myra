...
class SyncStatus:
    ...
    def update(self, task=None, completed=None, total=None):
        if task is not None: self.task_name = task
        if completed is not None: self.completed_count = completed
...
class LibrarianCore:
    ...
    def __init__(self, read_only=False, console=None, db_path=None):
        ...
        self.db_path = db_path if db_path else os.path.join(os.getcwd(), "db", "myra_market_data.db")
    ...
    def _connect_sqlite(self):
        ...
        configs = {
            '_tech_conn': "technical.db",
            '_inst_conn': "institutional.db",
            '_meta_conn': "meta.db",
            '_val_conn': "valuation.db",
            '_gov_conn': "governance.db"
        }

        for attr, db_name in configs.items():
            path = os.path.join(db_dir, db_name)
            # Create if not exists only if not read_only
            if not os.path.exists(path) and self.read_only:
                continue
            
            try:
                # We use check_same_thread=False for background sync compatibility
                conn = sqlite3.connect(path, check_same_thread=False)
                # Enable foreign keys and set WAL mode for performance
                conn.execute("PRAGMA journal_mode=WAL")
                conn.execute("PRAGMA synchronous=NORMAL")
                setattr(self, attr, conn)
            except Exception as e:
                print(f"[!] LibrarianCore: Failed to connect to {db_name}: {e}")
    ...
    def safe_execute(self, sql, params=None, conn=None, retries=5):
        ...
        if is_duck:
            for i in range(retries):
                try:
                    with self._db_lock:
                        if params: return c.execute(sql, params)
                        else: return c.execute(sql)
                except Exception as e:
    ...
    def get_metadata(self, key):
        if not self._meta_conn: return None
        try:
            res = self._meta_conn.execute("SELECT value FROM metadata WHERE key = ?", (key,)).fetchone()
            return res[0] if res else None
        except Exception: return None

    def set_metadata(self, key, value):
        if not self._meta_conn or self.read_only: return
        self._meta_conn.execute("INSERT OR REPLACE INTO metadata VALUES (?, ?)", (key, value))
    ...
    def close(self):
        ...
        conns = [self.conn, self._tech_conn, self._inst_conn, self._meta_conn, self._val_conn, self._gov_conn]
        for c in conns:
            if c:
                try: c.close()
                except Exception: pass
        self.conn = self._tech_conn = self._inst_conn = self._meta_conn = self._val_conn = self._gov_conn = None
